...
BASE_DIR      = str(PACKAGE_DIR / "data" / "glycoprotein_blast" / "three_species_target_blast")
...
# 全局预设
# ==============================================================================
BASE_DIR    = Path(__file__).resolve().parents[1] / "data" / "reglyco_ensemble"
PDB_DIR     = BASE_DIR / 'PDB'
CSV_DIR     = BASE_DIR / 'csv'
CSV_DIR.mkdir(exist_ok=True)
OLD_FOLDER  = Path(__file__).resolve().parents[1] / "external_tools" / "reglyco_apbs_helpers"

# 环境与参数
APBS_BINARY      = os.environ.get("APBS_BINARY", "apbs")
ENSEMBLE_STEP    = 5
CA2_PROBE_BARE   = 1.00
CA2_PROBE_HYD    = 4.12
APBS_THRESHOLD   = -5.0
...
def run_step_1():
    ...
    def load_gly_module():
        helper = OLD_FOLDER / "glycan_aware_apbs.py"
        if not helper.exists():
            raise FileNotFoundError(
                "APBS helper script is not included in this processed-data archive. "
                "Use the packaged CSV files in data/reglyco_ensemble/csv to reproduce "
                "the manuscript source-data tables, or provide glycan_aware_apbs.py "
                "under external_tools/reglyco_apbs_helpers for a full APBS rerun."
            )
        spec = importlib.util.spec_from_file_location("glycan_aware_apbs", helper)
