...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳是一个局部化失效过程：力不是分散到整个壳面，而是通过破壳齿传递到蛋壳内表面。", [16, 86]),
    ("类似的辅助出壳结构也见于其他卵生羊膜动物，因此具有生物学意义的出壳性能差异更可能位于蛋壳，而不是工具本身。", [16, 82, 83, 84, 85]),
    ("尽管这一框架清晰，我们目前仍缺少对核心问题的解释：在共享出壳力学约束下，不同生态与发育类型的物种为何形成不同蛋壳状态。", [15, 26, 39, 40, 41]),
    ("乳突层是最直接的切入点，因为它既是最早具有力学后果的蛋壳层，也是基质引导方解石生长的起点，局部分子差异可由此传播并放大为成熟蛋壳行为。", [1, 4, 20, 28, 30, 34, 57]),
])
...
p_intro2 = spara([
    ...
    ("因此，尚未解决的问题并不是工具箱是否存在，而是这一共享工具箱如何在物种间被差异化部署。", [1, 2, 4]),
    ("这一缺口在糖基化层面尤为明显。后修饰位点已经被记录，但不同于化学上较受限的磷酸侧链，糖链在组成、大小和电荷上变化更大，能够在同一蛋白支架上产生不同分子状态。", [17, 18, 21, 49, 50, 80]),
...
p_intro_sig_cn = spara([
    ("糖基化可改变蛋白稳定性、分子识别与折叠表面的可及性；在其他系统中，糖链被证明可作为动态遮蔽层重塑可接近界面。", [42, 43, 44, 61, 63, 72, 78, 81]),
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ([ ("这一设计在一个保守出壳框架内交叉发育与生态差异，而不把比较简化为单纯系统发育排序。", False, False)], []),
            ([ ("我们整合显微 CT 形态测量、比较蛋白组学与完整糖肽质谱、Re-Glyco 建模及静电分析和有限元模拟，构建从壳体结构到分子状态再到力学结局的跨尺度读出。", False, False)], []),
            ([ ("在这一序列中，OVAL 给出了最强且连贯的信号：其糖链状态与乳突密度、Ca²⁺ 相关表面可及性和局部出壳抗性一致。", False, False)], [18]),
])
