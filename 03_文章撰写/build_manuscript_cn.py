...
OUT = str(Path(__file__).with_name("manuscript260519_cn.docx"))
...
p_intro2 = spara([
    ...
    ("近期的蛋壳基质蛋白研究也反复回收到一组部分重叠的蛋白，尤其是 OC17、OC116、ovotransferrin 相关蛋白、OVAL、ovomucoid 以及 ovocalyxin 家族成员。这些分子的重复出现，使它们成为比较不同研究中蛋壳组织方式的自然起点。", [1, 2, 4, 19, 21, 29]),
    ("这种重复出现也说明，鸟类蛋壳并不是依赖彼此无关的蛋白清单来建成，而是在反复使用一套共享的基质蛋白工具箱。真正还未解决的问题，是这套共享系统在不同壳体背景中究竟如何被重新部署，以及关键差异到底落在蛋白丰度还是蛋白状态上。", [1, 2, 4]),
...
p_intro_sig_cn = spara([
    ("这一缺口之所以重要，是因为糖基化会影响蛋白稳定性、分子识别、表面暴露和构象状态，而其他系统中的研究也已表明，糖链可以作为动态遮蔽层，而不是被动附着的体积负担。", [42, 43, 61, 63, 72, 78]),
    ("Zeng Lingsen 等人的蛋壳 N-糖蛋白组工作进一步表明，同一个基质蛋白在 cuticle 与 mineralized layer 中可以对应不同的糖基化状态，这意味着糖链状态不仅是化学修饰，还可能帮助把同一蛋白的生物学功能重新分配到不同壳层区室。", [18]),
    ("既往矿化相关工作进一步提示，OVAL 在早期成壳阶段可能进入 Ca²⁺ 响应的构象状态，因此糖链依赖的表面可及性就成为一个可以检验的机制变量。如果糖链差异会改变同一共享基质蛋白呈现折叠表面的方式，那么结构建模就应揭示矿化起始时呈现给 Ca²⁺ 的表面差异。", [4, 11, 29, 42, 43, 61, 63, 81]),
    ("而如果这种结构差异具有生物学意义，那么它也应当在出壳相关的力学终点上保留下来，即表现为破壳齿样加载下乳突界面的局部抗性差异。", [16, 37, 69]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("因此，我们把比较锚定在保守的破壳齿界面上，并检验共享基质蛋白上的糖链状态差异是否能够解释，同一套成壳工具箱为何会在矿化起始时呈现不同的 Ca²⁺ 可接近状态。放在这一框架下，真正缺少的并不是另一份蛋白清单，而是一座从糖链类别通向共享基质表面呈现方式的桥梁。", [4, 18, 29, 42]),
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ([ ("这一设计避免了比较坍缩为简单的系统发育配对，或单一的“早成对晚成”对照。", False, False)], [3, 22, 23]),
...
p_ss1_cn = smixed([
    ...
    ([('这一更宽的比较定位之所以必要，是因为与蛋最直接相关的两条轴线并不是简单二值分类：一条是巢穴环境，它与陆生到强水域依赖的梯度高度相关；另一条是雏鸟状态，它从更早成到更晚成连续分布。', False, False)], [15, 16, 22, 23, 24, 25, 41]),
    ([('在这一比较空间中，', False, False),
        ...
        ('和', False, False),
        ('Columba livia', False, True),
        ('因此被选作后续比较对象，不是因为它们构成整齐类别，而是因为它们在这些连续生态—发育梯度上被有意拉开，从而尽量减少中间组合对比较解释的干扰。', False, False)], [15, 22, 23, 24, 25, 41]),
...
p_sprot_focus_cn = spara([
    ...
    ("从与前人蛋壳基质蛋白结果的对应关系看，我们的数据总体上是吻合的，但重合并不均匀。OVAL、OC116 和 TRFE 在严格直系同源过滤后仍保留为三物种共享糖基化锚点，OC17 只保留在鸡中，而 ovomucoid、lactadherin 和 clusterin 只出现在谱系偏向或两两共享部分。因此，本研究对既有工作的扩展，主要不是显著放大核心蛋壳基质蛋白名单，而是把一组部分共享的基质蛋白推进到了糖基化状态层面的比较。", []),
])
...
p_s2c_cn = spara([
    ("整合蛋白丰度和糖链丰度后，图3B-D进一步识别出OVAL是与跨物种蛋壳差异最一致的共享蛋白。在全数据集中，鸡的蛋白-糖链耦合较弱，而鸭和鸽则持续为正，说明三条谱系不仅糖链身份不同，糖基化与蛋白输出的对应关系也不同。", []),
    ("在重点蛋壳基质蛋白中，OVAL在三物种中都保持高丰度，但其糖链负荷变化明显：鸡最低，鸭更高，鸽最高。相比之下，OC116和TRFE虽然仍然有信息量，但并不像OVAL那样稳定地区分总体蛋白丰度与糖链输出。", []),
    ("图3E-G中的两两富集图进一步说明了为什么OVAL是最清晰的候选。在 Gallus 对 Anas 和 Gallus 对 Columba 的比较平面中，OVAL都落在糖链变化相对更突出的区域，说明它的糖链偏移并不是简单跟随蛋白丰度一起移动，而是比蛋白丰度变化更快，甚至与之部分脱耦。在 Anas 对 Columba 的比较中，OVAL仍然偏离蛋白-糖链完全一致的对角关系，从而把同一排序延续到鸡之外的比较中。完整糖肽鉴定因此把 OVAL 放入一条连贯的跨物种糖型序列：鸡以紧凑高甘露糖型为主，鸭转向中性复合/杂合型，鸽则进一步转向更延伸的唾液酸化复合/杂合型。综合来看，图3B-G将OVAL界定为糖基化变化与表型联系最稳定的共享蛋白。", []),
])
...
p_s3a_cn = spara([
    ...
    ("但一旦去除糖链，这种分离中的大部分就会明显收敛，三物种蛋白骨架彼此更接近。因此，最初被恢复出的结构差异并不是一般性的序列差异，而是糖链把矿化起始时暴露给离子环境的酸性表面重新组织了出来（图4A-C；图S7）。", []),
])

p_s3b_cn = spara([
    ("接下来的问题是，这种糖链状态推进在几何上究竟如何落实到 OVAL 表面。鸽首先通过更大的整体糖链包络与另外两种鸟分开，这一点体现在图4D更高的回转半径和图4E更长的端到端距离上。", []),
    ("但这种扩展并没有把糖链整体抬离蛋白表面。相反，在图4F中，鸽的糖链-蛋白距离并未相应增大，而在图4G中还表现出更小的最小糖链-骨架距离，说明这些更长的糖链同时也更容易回折并贴近 OVAL 表面。鸡对应的是另一端：图4D和图4E中的糖链最紧凑，图4F和图4G中的表面接触也最弱；鸭则再次位于两者之间。这样，图4D-G就把跨物种糖型差异转化为一条遮蔽几何链条：从鸡的紧凑且弱接触糖链，到鸽的延伸但贴面糖链，再到居中的鸭状态。", []),
])
p_s3c_cn = spara([
    ("图4H-K进一步把同一片酸性界面放到连续过滤之下。图4H显示界面遮蔽从鸡到鸭到鸽逐步增强。图4I显示暴露的热点表面积按同样顺序下降，图4J显示热点比例进一步下降，图4K则显示净 Ca²⁺ 可及热点数量也按同样顺序减少。", []),
    ("这些并不是彼此分离的指标，而是同一遮蔽过程的连续读数。结果说明，随着糖链从紧凑转向延伸且贴面，同一片 OVAL 酸性表面在矿化起始时会逐步失去化学可用性。", []),
])

p_s3d_cn = spara([
    ("图4L和图4M则把同一差异进一步压缩到整体界面层面，分别把热点数量和热点残基表面积拆分为暴露部分与被遮蔽部分。鸡在这两幅图中都保留了最大的暴露份额，鸽把最大的份额转入遮蔽区室，而鸭仍位于两者之间。", []),
    ("同样的汇总也把鸡放在最强的推定 Ca²⁺ 吸附端，因此最符合更早进入 Ca²⁺ 响应 OVAL 构象、并更早开始矿化的状态。鸽对应另一端，鸭仍位于中间。这个排序与前文表型结果一致：鸡同时具有最高的乳突密度和最高的局部出壳抗性，而鸭和鸽则落在较低抗性一侧。综合来看，图4A-M构成的是一条连续论证：从糖链依赖的分离，到糖链几何、界面遮蔽，再到 Ca²⁺ 相关表面可及性。", []),
])

head("有限元分析把同一对比连接到局部出壳抗性")

p_s4a_cn = mixed([
    ("有限元检验把共同的破壳齿界面转化为显式的内向外加载设计。图5A给出了与出壳相关的加载背景，图5B-D则把三种鸟类喙部俯视图与对应的micro-CT有限元建模并列展示。由于网格保留了来自micro-CT的物种特异性壳体几何，这一力学检验始终锚定在前文已经识别出的乳突层背景之上，而不是理想化均质壳壁模型。冲击加载在圆形蛋壳碎片（模型直径D = 2.0 mm）的9个参数化横向偏移位置上采样（3 × 3网格；间距0.5 mm），每物种获得n = 9条独立接触剪切应力时间曲线。为尽可能降低模型尺寸、整体几何以及尤其是蛋壳厚度差异对结果的影响，我们同时记录原始峰值接触力F_max和峰值接触剪切应力τ_max。本文将τ_max视为乳突接触界面局部出壳抗性的直接读数，并从有限元单元输出中"
     "直接提取各偏移位置的峰值接触剪切应力τ_max，"
     "然后计算9个位置的物种均值 ± s.d."
    "（图S8A-F；壳厚分别为", False, False),
    ("G. gallus", False, True),
    (" 0.29 mm、", False, False),
    ("A. platyrhynchos", False, True),
    (" 0.35 mm、", False, False),
...
p_disc_mam1_cn = smixed([
    ([('本研究把一个保守的出壳界面与乳突层组织、共享基质蛋白上的糖链状态变化以及局部出壳力学连接起来。由于三种鸟的破壳齿功能本身保守，最有信息量的差异只能在蛋壳中寻找，首先出现在乳突层，随后进一步收束到共享基质背景上的糖基化。', False, False),
            ('在这条序列中，OVAL 糖链状态为分子变化、表面可及性和局部出壳抗性之间提供了当前数据中最可解释的对应关系。', False, False)], [1, 16, 18]),
])
...
p_disc_selection_cn = para(
    "鸭和鸽之所以仍然关键，在于它们从壳体结构和生态—发育位置两个层面共同界定了鸡状态的边界。鸭对应更偏水域、且广义上更早成的一端，同时具有更厚的壳体、中间型 OVAL 可及性和较低的 τ_max，这说明单靠壳厚并不能重建鸡状态。鸽则从更薄的壳体、更偏陆生的背景和更晚成的位置，收敛到同样较低的 τ_max 终点。正是这两个对照，让鸡成为当前生态—发育取样空间中、最适合用来连接糖链依赖基质行为与蛋壳性能的参考状态。在这一框架下，OVAL 糖链状态是让鸡式高抗性状态获得机制解释的最明确分子层。",
    bold=False, size=11, before=0, after=120
)

p_disc_biomineral_cn = para(
    "同样的分析序列也应当可以用于鸟类蛋壳之外的体系。许多生物矿化系统都依赖有机基质通过化学特异性的界面状态来调节离子可及性、表面暴露和矿物成核。放在这一更广的框架下，本研究提供的是一条从糖蛋白状态通向表面呈现、再通向介观功能的分析路径。它也可能对再生医学场景有参考价值，因为蛋壳来源材料和蛋壳膜蛋白已经被用于组织工程与骨修复，包括提高注射型骨移植物的成骨效率。因此，这项工作的意义并不只局限于一种鸟类蛋壳表型，也在于它提供了一个讨论“基质状态如何组织矿化行为”的可迁移框架。",
    bold=False, size=11, before=0, after=120
)
