...
p_s2a_cn = spara([
    ("完整糖肽分析显示，三物种在采样深度上存在差异，但仍共享一个稳定的比较核心（图2A至D）。在cluster层面，三物种共有25个共享cluster，而最大的额外两两重叠来自鸭和鸽的64个cluster；鸡几乎没有形成明显的物种私有cluster空间（图2A）。", []),
    ("同样的模式也出现在清单数量上：鸭检测到321个糖蛋白、547个糖基化位点和197种糖链组成；鸽分别为192、257和162；鸡分别为55、88和105（图2B）。尽管覆盖深度不同，共享核心的Jensen-Shannon相似性仍集中在0.33到0.40之间，其中鸭-鸽配对最高。这说明三物种的差异发生在一个仍可比较的糖蛋白背景之内，而不是彼此割裂的三个化学空间（图2C）。", []),
...
p_disc_other_cn = spara([
    ("非OVAL信号仍然重要。OC116和TRFE仍是有信息量的共享蛋白，而OC17仅在鸡中显示糖基化，因此可能代表一种更具谱系限制性的成矿程序。", [10, 19, 29]),
