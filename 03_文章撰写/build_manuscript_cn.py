...
p_disc_future_cn = para(
    "本研究范围仍有边界。我们分析的是主导糖型而非体内完整糖链系综；同时在平均蛋壳尺度上把各物种视为机械均一，并在 APBS 框架下使用约束尚不充分的子宫离子条件。下一步关键检验包括：定义糖型的矿化实验、在Gallus中直接操控 OVAL 糖基化，以及对同一内向外力学对比进行位点分辨验证。这些实验将澄清 OVAL 糖链状态是直接参与壳体矿化，还是以异常高保真度标记Gallus式高抗性状态。",
    bold=False, size=11, before=0, after=120
)
