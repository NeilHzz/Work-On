...
p_s3d_cn = spara([
    ("配对的糖基化与apo对照进一步显示，加上糖链后Ca²⁺相关热点残基数量和暴露羧酸基表面发生改变，说明恢复出的结构差异并不是一般性的序列差异，而是糖链把矿化起始时暴露给离子环境的酸性表面重新组织了出来（图4K和L；图S10）。图4K至N随后利用匹配的糖基化和apo参考，把同一对比压缩到整体界面层面。热点数量、羧酸基表面可及性、净热点可及性和热点残基SASA都保留了相同的定性排序。鸡保留了最多的Ca²⁺相关可及表面，鸽把最大的份额转入糖链影响状态，鸭则依指标不同处于中间或与鸽收敛。", []),
