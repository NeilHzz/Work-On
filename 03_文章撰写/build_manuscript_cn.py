...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳是一个局部化失效过程：力并非分布于整个壳面，而是通过一个由破壳器定义的界面传递。在鸟类中，这个破壳器就是短暂存在的破壳齿，在出壳时顶压蛋壳内表面。", [16, 86]),
    ("类似的辅助出壳结构也见于其他卵生羊膜动物，这表明真正有生物学意义的出壳性能差异更可能位于蛋壳，而不是工具本身。", [16, 82, 83, 84, 85]),
    ("尽管如此，鸟类蛋壳仍会随巢穴环境、气体交换需求、微生物暴露和发育方式而变化，因此比较研究恢复出的不是单一通用结构，而是多种蛋壳解决方案。", [15, 26, 39, 40, 41]),
    ("乳突层尤其关键，因为它既是最早具有力学后果的蛋壳层，也是基质调控下方解石开始生长的位置；后续壳层会继承这一最初成矿背景。", [1, 4, 20, 28, 30, 34]),
    ("因此，乳突层组织就是局部分子差异最早可能被传播并放大为成熟蛋壳行为的层级。", [1, 57]),
])

p_s1b_cn = spara([
    ("因此，真正的机制问题就变成：当破壳界面被固定后，究竟是哪一类调控乳突层组织的分子因素，能够解释不同物种之间恢复出的不同蛋壳状态？", [1, 4, 16]),
    ("既往关于蛋壳基质与乳突层的研究，已经把这一层级置于最可能的控制点。", [2, 28]),
])

# §2 — 已知工作与局限
p_intro2 = spara([
    ("蛋壳基质蛋白调控乳突层成矿、晶体生长和成熟壳体结构，而 OC17、OC116、TRFE 和 OVAL 等反复出现的因子共同定义了一套广泛共享的成壳工具箱。", [1, 2, 4, 10, 19, 21, 29]),
    ("因此，真正尚未解决的问题并不是工具箱是否存在，而是这一共享工具箱如何在物种间被差异化部署。", [1, 2, 4]),
    ("这一缺口在糖基化层面尤为突出。后修饰位点已被较充分记录，但磷酸侧链在化学上相对受限，而糖链在组成、大小和电荷上变化极大，因此能在同一蛋白支架上产生不同分子状态。", [17, 18, 21, 49, 50, 80]),
    ("因此，糖基化分析不能停留在位点占据层面；糖链类别本身就是机制变量。", [49, 50]),
    ("早期糖蛋白组学和生化研究已证明，蛋壳基质蛋白可以处于不同 N-糖基化状态，包括 OC116 的糖基化 Asn 以及 OVAL 相关糖链的组成定义。", [8, 18, 21]),
    ("然而，多数研究都按单一物种、单一区室或单个位点目录组织，因此跨物种共享基质蛋白所携带的匹配糖链状态信息仍然稀缺。", [7, 47, 48]),
...
p_intro_sig_cn = spara([
    ...
    ("在其他系统中，糖链并非被动体积，而是动态遮蔽层；位点特异性的糖链状态可以重塑折叠蛋白表面并改变可接近界面。", [42, 43, 44, 61, 63, 81]),
    ("Zeng 及其同事进一步表明，同一种蛋壳基质蛋白在角质层和矿化层之间可以处于不同的 N-糖基化状态。因此，糖链状态可能重新分配蛋白在不同蛋壳区室中的生物学角色，而不是仅仅装饰一个固定的蛋白支架。", [18]),
    ("在这一背景下，OVAL 尤其关键，因为既往矿化研究提示它在早期成壳过程中可能进入 Ca²⁺ 响应构象状态。", [4, 11, 29]),
    ("因此，我们进一步检验跨物种糖链差异是否会重塑折叠 OVAL 的表面、改变矿化起始时呈现给 Ca²⁺ 的可接近界面，并在与出壳相关的力学终点上继续留下信号。", [4, 16, 18, 37, 69]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("因此，我们把比较锚定在保守的破壳齿界面上，并检验共享基质蛋白上的糖链状态差异，是否能够解释为什么同一套成壳工具箱会在矿化起始时产生不同的 Ca²⁺ 可接近状态。", []),
    ("在这一框架下，真正缺失的一步，是把糖链类别直接连接到共享基质背景上的表面呈现方式。", []),
    ("OVAL 提供了一个便于检验的案例，因为既往矿化工作已提示其 Ca²⁺ 响应表面行为具有生物学意义，它在三物种中保持高丰度，其主导糖链类别也可以从糖蛋白组学一路追踪到结构建模。", [4, 18, 29, 42]),
])
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ([ ("这一设计在一个保守的出壳框架内抽样发育与生态的交叉对比，避免把比较压缩为单纯系统发育排序或简单的早成/晚成二分。", False, False)], []),
            ([ ("我们用显微 CT 形态测量刻画乳突层组织，用比较蛋白组学与完整糖肽质谱解析共享基质蛋白及其糖链状态，用 Re-Glyco 建模与静电分析推断表面后果，并用有限元模拟检验同一对比是否在力学层面持续存在。", False, False)], []),
            ([ ("这种分步设计把解释约束在跨层证据链上：从壳体结构，到分子状态，再到力学结局。", False, False)], []),
...
p_disc_oval_cn = spara([
    ...
    ("因此，本研究把糖链状态变化解析为可进行物理解释的表面差异。尽管该结果不构成直接因果证明，但它支持一个克制推论：同一基质蛋白上的不同糖链状态可改变呈现给矿化环境的化学表面，并可能参与这里观察到的结构分化。", [42, 61]),
], before=0, after=120)

p_disc_mech_cn = spara([
    ("本研究的力学比较围绕出壳过程中的内向外加载事件设计，而非常规外压或整壳破裂测试。", [16, 37, 69]),
    ...
    ("鸭最清楚地体现了这种分离：更厚壳体抬高了 F_max，却未重建鸡所呈现的高 τ_max 状态。", [16, 37, 69]),
])
