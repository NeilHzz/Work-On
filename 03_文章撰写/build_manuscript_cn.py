...
p_sprot_go_cn = spara([
    ("GO富集和基因家族周转进一步提示了谱系特异的免疫与防御背景（图S5、S8和S9），但这些信号更多描述的是比较背景，而不是与乳突层组织最直接对应的层级。这些带有谱系偏向的信息仍然是重要的演化背景，但它们本身并没有指出把共享基质体系与乳突层分化及后续出壳相关力学连接起来的最近端层级。", []),
])
...
p_s2a_cn = mixed([
    ("完整糖肽为鸟类蛋壳基质蛋白上的具体糖型提供了直接的跨物种视角。图2A至C首先说明，三物种糖蛋白组并不是以相同深度被采样的，但它们仍共享一个稳定的比较核心。在cluster层面，三物种共有核心包含25个共享cluster，而最大的额外重叠来自鸭-鸽配对的64个cluster；鸡特异cluster在这一聚类视图中几乎缺失，说明鸡样本主要是通过共享基质背景进入比较，而不是依赖一个庞大的私有糖蛋白分区（图2A）。同样的不对称也出现在定量清单中：鸭的覆盖最广，检测到321个糖蛋白、547个糖基化位点和197种糖链组成；鸽居中，为192、257和162；鸡最紧凑，为55、88和105（图2B）。尽管覆盖广度不同，共享核心的Jensen-Shannon相似性仍集中在0.33到0.40之间，且鸭-鸽配对最高，说明三物种并不是落在彼此割裂的化学空间中，而是在一个可比较的共享糖蛋白框架上展开差异（图2C）。糖链类别组成进一步显示，High Mannose和Complex-Fucosylated糖链构成跨物种广泛背景，而Complex-Sialylated及其他更延伸类别更明显地参与谱系差异（图2D）。", False, False),
    ("直系同源-糖链弦图则在蛋白层面保留同一信息：蛋壳基质蛋白工具箱总体共享，但单个共享蛋白在不同物种中携带的优势糖链类别不同（图2E）。这一不对称性由此突出了一组状态差异显著的共享糖蛋白，并把后续直系同源和结构分析的候选范围进一步收窄。", False, False),
])
...
p_s2c_cn = spara([
    ("整合蛋白丰度和糖链丰度后，图3A至C进一步识别出OVAL是与跨物种蛋壳差异最一致的共享蛋白。在全数据集中，鸡的蛋白-糖链耦合较弱，而鸭和鸽则持续为正，说明三条谱系不仅糖链身份不同，糖基化与蛋白输出的对应关系也不同。", []),
    ("在重点蛋壳基质蛋白中，OVAL在三物种中都保持高丰度，但其糖链负荷变化明显：鸡最低，鸭更高，鸽最高。相比之下，OC116和TRFE虽然仍然有信息量，但并不像OVAL那样稳定地区分总体蛋白丰度与糖链输出。", []),
    ("图3D至F中的两两富集图进一步说明了为什么OVAL是最清晰的候选。在 Gallus 对 Anas 和 Gallus 对 Columba 的比较平面中，OVAL都落在糖链变化相对更突出的区域，说明它的糖链偏移并不是简单跟随蛋白丰度一起移动，而是比蛋白丰度变化更快，甚至与之部分脱耦。在 Anas 对 Columba 的比较中，OVAL仍然偏离蛋白-糖链完全一致的对角关系，从而把同一排序延续到鸡之外的比较中。完整糖肽鉴定因此把 OVAL 放入一条连贯的跨物种糖型序列：鸡以紧凑高甘露糖型为主，鸭转向中性复合/杂合型，鸽则进一步转向更延伸的唾液酸化复合/杂合型。综合来看，图3A至F将OVAL界定为糖基化变化与表型联系最稳定的共享蛋白。", []),
])
...
p_s3b_cn = spara([
    ("接下来的问题是，这种糖链状态推进在几何上究竟如何落实到 OVAL 表面。鸽首先通过更大的整体糖链包络与另外两种鸟分开，这一点体现在图4C更高的回转半径和图4E更长的端到端距离上。与此同时，图4D中更近的局部骨架接近距离和图4F中更宽的糖链-蛋白距离分布说明，这些更长糖链既探索更大的空间包络，也能回折并接近 OVAL 表面。", []),
    ("鸡对应的是另一端，表现为更紧凑、对酸性界面几何侵入最弱的糖链层；鸭则再次位于两者之间。这样，图4C至F就把跨物种糖型差异转化为一条遮蔽几何链条：从鸡的紧凑且弱接触糖链，到鸽的延伸且表面参与更强的糖链，再到居中的鸭状态。", []),
])
p_s3c_cn = spara([
    ("一旦这种几何排序被建立，图4G至J要回答的就不再是四个彼此无关的问题，而是同一片共享酸性界面在连续过滤后还剩下多少可用于矿化。图4G先给出整体界面遮蔽程度；图4H进一步问候选酸性残基中还有多大比例仍能保留为热点；图4I再问热点残基还有多少表面积保持溶剂暴露；图4J最后只保留其中同时满足静电有利且空间可及、能够被Ca²⁺接近的净热点。", []),
    ("按这个顺序阅读，图4G至J构成的是一条单向推进的遮蔽级联。界面遮蔽从鸡到鸭到鸽逐步增强，而每一步下游筛选都保留同样的排序，说明更延伸且更贴面的糖链会把同一片共享酸性 OVAL 表面逐步从“大面积可用界面”压缩成更小、更难被矿化环境利用的化学活性区。", []),
])

p_s3d_cn = spara([
    ("图4K至N则利用匹配的糖基化和apo参考，把同一差异进一步压缩到整体界面层面。热点数量、羧酸基表面可及性、净热点可及性和热点残基SASA都保留了相同的定性排序：鸡保留最多Ca²⁺相关可及表面，鸽把最大的份额转入糖链影响状态，鸭则根据指标不同处于中间或与鸽收敛。", []),
    ("把这一结果压缩到整体界面层面后，鸡也就对应于最强的 Ca²⁺ 吸附潜力，并且最符合更早进入 Ca²⁺ 响应式构象打开、从而更早启动矿化的状态；鸭和鸽则从不同结构背景转向较低可及性侧。这一排序与前文表型链条是对上的：鸡同时具有最高的乳突密度和最高的局部出壳抗性，而鸭和鸽则收敛到较低抗性侧。因此，图4A至N构成的是一条连续的结构论证，而不是若干并列子图：它从糖链依赖的分离出发，经过糖链几何和界面遮蔽，最后收束到共享基质蛋白上的 Ca²⁺ 相关表面可及性。", []),
])
