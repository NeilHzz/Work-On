...
p_s2a_cn = mixed([
    ("完整糖肽为鸟类蛋壳基质蛋白上的具体糖型提供了直接的跨物种视角。图2首先概括了糖蛋白组学数据的整体搜索空间，显示三物种在cluster数量、糖蛋白覆盖、糖基化位点数和糖链组成数上存在差异，但仍保留可比较的共享核心信号（图2A至C）。糖链类别组成进一步显示，High Mannose和Complex-Fucosylated糖链构成跨物种广泛背景，而Complex-Sialylated及其他更延伸类别更明显地参与谱系差异（图2D）。", False, False),
