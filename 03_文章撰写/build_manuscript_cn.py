...
OUT = str(Path(__file__).with_name("manuscript260506v3_cn.docx"))
...
# ════════════════════════════════════════════════════════════════════════════
para(
    "基质蛋白糖链状态分化联系鸟类蛋壳生物矿化与局部出壳功能",
    bold=True, size=14, before=0, after=160,
    align=WD_ALIGN_PARAGRAPH.LEFT
)

para(
    "Short title: 糖链状态联系蛋壳矿化与出壳",
...
para(
    "在卵生羊膜动物中，承担破壳功能的结构虽在形态上可变，但在力学上都收敛到局部出壳界面；在鸟类中，这一界面由破壳齿从壳内侧顶压蛋壳来定义。"
    "比较生物矿化研究常能揭示谱系间差异，却较少确定最贴近有序表型的化学特异性分子层。"
    "这里，我们整合显微CT形态测量、蛋壳基质蛋白组学、完整糖肽质谱、结构系综建模和有限元分析，对鸡、鸭和鸽进行比较，发现模拟破壳齿加载下的局部出壳抗性重现了乳突层级排序，将鸡与鸭、鸽区分开来。"
    "进一步的比较糖蛋白组学表明，基质蛋白糖链状态存在稳定的物种排序，其中以卵白蛋白最为清晰：鸡以高甘露糖型为主，鸭转向中性复合/杂合型，鸽进一步转向唾液酸化复合/杂合型。"
    "Re-Glyco与静电分析进一步表明，这一变化对应Ca²⁺可及表面的逐步降低，并与成矿方式和局部力学结果的总体排序一致。"
    "综合来看，基质蛋白糖链状态分化构成了一个具有明确化学含义的比较层，用于连接分子差异与鸟类蛋壳的结构、生物矿化和局部出壳功能。",
    bold=False, size=10, before=0, after=80, align=WD_ALIGN_PARAGRAPH.JUSTIFY
)

para(
    "Teaser: 破壳齿加载揭示蛋壳糖基化如何对应局部出壳抗性。",
...
# §1 — 背景价值
p_s1a_cn = spara([
    ("有一句流行箴言说：鸡蛋从外打破，是食物；从内打破，是生命。", []),
    ("对于鸟类而言，这种从“食物”到“生命”的转折并不是抽象比喻，而是依赖一个短暂存在的破壳结构——破壳齿。它把胚胎施加的力聚焦到蛋壳内侧的局部接触界面上，并触发出壳过程。", [16, 17, 38, 82, 86]),
    ("如果从这一共享的出壳界面反向阅读蛋壳，那么蛋壳就不只是保护性的外壳，而是记录基质化学、微结构组织与局部力学如何在发育过程中被协调起来的生物矿化结构。", [1, 4, 16, 20, 21, 38, 42]),
    ("它既保护蛋不受外界病原微生物侵染，也为胚胎提供必要的钙来源，而其力学性能首先在乳突层被设定。", [1, 4, 28, 38]),
...
p_s1b_cn = spara([
    ("在这一以出壳为中心的视角下，由发育策略和生境共同塑造的繁殖环境，又对蛋壳需要提供怎样的结构防护提出了不同要求。", [15, 16, 23, 26, 41]),
...
p_intro_sig_cn = spara([
    ("这一问题在把蛋壳放回出壳过程时会变得尤其清晰。", [16, 17, 38]),
    (" 在卵生羊膜动物中，胚胎通常借助专门的破壳辅助结构完成出壳。", [82, 83, 84]),
...
p_s0b_cn = mixed([
    ...
    ("G. gallus", False, True),
    ("最低（0.3975 ± 0.0127）。这些结果说明，鸡形成了最致密的乳突成核场，鸽由单个乳突 knobs 启动并向外扩展的晶体单元在整个蛋壳中占比最高，而鸭在晶体单元占比上居中、在密度上则与鸽更为接近。接下来的问题因此是，这种有序形态差异究竟对应蛋壳基质工具箱的整体更替，还是共享系统内部的差异化部署。由于已知蛋壳基质蛋白是调控蛋壳微观结构的关键因素，我们随后开展了系统性的比较蛋白质组学分析，以回答这一点。", False, False),
])
...
p_s3a_cn = spara([
    ...
    ("既然已经确定拉开三物种差异的是糖基化，而不是蛋白骨架本身，进一步需要解析的就是不同糖链究竟通过哪些具体性质影响OVAL。图4D-G直接回答的是糖链层面的差异：鸽的再糖基化糖链占据了最大的构象空间，平均回旋半径和端到端距离最高（Rg 9.13 Å；端到端距离22.30 Å），而鸡和鸭保持更紧凑状态（Rg 7.50和7.61 Å；端到端距离18.05和17.52 Å）。相比之下，糖链质心到蛋白的平均距离在三物种间变化不大（约30.2-30.9 Å），但糖链与蛋白表面的最小距离却从鸡到鸽逐步减小（3.58、3.26和3.00 Å），说明更长的鸽糖链并不是简单向外伸展，而是在更大构象体积下仍维持与蛋白表面的更频繁接触。图4H-K继续把这种几何差异转译为遮蔽效应：界面遮蔽面积从鸡到鸭到鸽依次增加，而可保留为Ca²⁺热点的酸性残基比例则按相反方向下降，净可及热点数也进一步被压缩。图4I和4M则补充说明，热点残基平均SASA及其在“净可及”和“糖链遮蔽”之间的分配与上述趋势一致，但仍保留了不同物种在表面积分配方式上的细微差异。这里的SASA并不是广义的全蛋白SASA，而是满足Ca²⁺热点定义的表面Asp/Glu残基的可及面积。由此，图4结合图S7把糖链几何、表面遮蔽、热点数量和热点面积压缩到同一套结构框架中。具体到物种排序，鸡始终代表OVAL Ca²⁺可及性最高的状态，鸭处于中间或在某些汇总指标上向下收敛，而鸽则总体上保持最强遮蔽。更高的Ca²⁺可及性意味着鸡的OVAL更容易进入既往研究提示的部分解折叠状态，从而可能促进更早的初始矿化介入，这一点也与鸡中观察到的更高乳突knob密度保持一致。", [11, 12, 42, 43, 44, 45, 46, 49, 50, 51]),
])

head("局部出壳抗性保留同一排序")

p_s4a_cn = mixed([
    ("结果开头所界定的共同破壳齿界面，随后被进一步转化为有限元检验中的显式加载设计。图5A给出了与出壳相关的加载背景，而图5B-D则把三种鸟类喙部俯视图与对应的micro-CT有限元建模并列展示，其中虚线框标示破壳齿位置，其喙尖几何基础已在图1B中概括。既往研究已经较好地利用壳体外侧加载测量整壳强度，但这里更相关的读数是出壳过程中乳突界面的局部响应，因此本文采用了不同的加载逻辑。我们关注的并不是广义整壳强度，而是乳突接触界面的局部抗性是否仍与前文恢复出的分子和结构排序保持一致。换句话说，这里的有限元分析承担的是一个跨尺度检验：前文由OVAL可及性和乳突组织所推得的排序，是否仍能在真实出壳过程中经历的力学界面上被检测出来。"
...
mixed([
    ...
    "即便它并不具备相同的高密度乳突状态。τ_max聚焦于micro-CT保留的乳突接触界面局部出壳抗性，"
     "因此消除了这种表观歧义，并显示高密度鸡状态在功能上仍然独立，而鸭与鸽则在较低抗性上收敛。这样一来，这组有限元结果就不只是补充一般性的力学描述，而是在力学尺度上呼应了前文所见的糖基化相关差异与乳突组织变化，并把这种一致性延伸到局部出壳抗性这一功能终点。", False, False),
])
