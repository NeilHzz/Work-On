...
p_s3c_cn = spara([
    ("一旦这种几何排序被建立，下一步就要看共享酸性界面究竟还剩下多少可用于矿化环境。图4H中的界面遮蔽程度从鸡到鸭到鸽逐步升高，而图4I中的热点残基溶剂可及表面积、图4J中的热点比例以及图4K中同时保持电势有利且空间可及的净热点数量，则沿相反方向逐步下降。", []),
    ("因此，图4H-K并不是几项彼此独立的读数，而是一条同向推进的遮蔽级联：随着糖链几何从紧凑转向延伸且贴面，同一片共享酸性 OVAL 界面会变得更受遮蔽、更少暴露，也更不容易在早期成矿时保持化学可用。", []),
])

p_s3d_cn = spara([
    ("图4L和图4M则把同一差异进一步压缩到整体界面层面，分别把热点数量和热点残基表面积拆分为暴露部分与被遮蔽部分。鸡在这两幅图中都保留了最大的暴露份额，鸽把最大的份额转入遮蔽区室，而鸭仍位于两者之间。", []),
    ("到这里，整条链条才真正闭合：鸡的高甘露糖型使酸性界面保持最开放，鸭的中性复合/杂合型使其部分覆盖，而鸽的唾液酸化复合/杂合型则造成最强的整体遮蔽。因此，图4A-M构成的是一条连续的结构论证，而不是若干并列子图：它从糖链依赖的分离出发，经过糖链几何和界面遮蔽，最后收束到共享基质蛋白上的 Ca²⁺ 相关表面可及性。", []),
])
