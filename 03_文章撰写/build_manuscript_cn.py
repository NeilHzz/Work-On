...
p_disc_function_cn = para(
    "因此，这项工作的更大意义首先是方法论上的：在当前数据中，一个共享基质蛋白上的化学特异性翻译后状态，帮助补上了蛋壳结构与局部功能之间缺失的那一层。这样的中介层在生物矿化研究里尤其重要，因为广义蛋白组变化往往比真正偏置晶体生长的具体化学状态更容易被首先看到。类似的思路也可能超出鸟类蛋壳，因为许多矿化系统同样依赖高丰度基质蛋白，而这些蛋白的翻译后状态可以在不整体更换蛋白库的情况下发生显著变化。我们并不是把蛋壳分化压缩成一个单分子故事，而是说明同一套保守蛋白工具箱可以通过糖链状态的再部署，导向不同的结构结果。",
    bold=False, size=11, before=0, after=120
)
