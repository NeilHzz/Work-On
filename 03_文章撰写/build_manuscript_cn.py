...
para(
    ...
    "我们在一个保守的破壳齿界面下比较Gallus、Anas和Columba，整合了显微 CT 形态测量、蛋壳基质蛋白组学、完整糖肽质谱、Re-Glyco 结构建模、静电分析和有限元模拟。"
    "跨物种分离首先出现在乳突层组织，而基质蛋白工具箱整体仍然大体共享。"
    "在这一共享背景下，卵清蛋白 OVAL 从Gallus中以高甘露糖型为主，转变为Anas中以中性复合/杂合型为主，以及Columba中以唾液酸化复合/杂合型为主。"
    "这些糖链状态预测了 OVAL 钙相关表面的逐步增强遮蔽，并在局部出壳抗性中对应为Gallus高、Anas/Columba低的对比。"
    "综合来看，数据将 OVAL 糖链状态定位为连接Gallus式蛋壳状态、乳突层组织和内向外失效行为的一个分子层。",
    bold=False, size=10, before=0, after=80, align=WD_ALIGN_PARAGRAPH.JUSTIFY
)

para(
    "导语：跨物种 OVAL 糖链状态揭示了Gallus式高抗性蛋壳背后的分子轴线。",
...
p_ss1_cn = smixed([
    ...
    ([('这种功能性分组只与系统发育部分重叠。Gallus和Anas仍是亲缘较近的早成类群，但沿栖息环境轴分开；Columba则锚定了比较中的晚成端点（图S2）。', False, False)], [3, 22, 23]),
    ([('因此，这一比较保留了共同祖先背景，同时在同一框架内分开了生活史差异。', False, False)], []),
...
p_s0b_cn = mixed([
    ...
    ("Gallus", False, True),
    ("最低（0.40 ± 0.01）。在这一扫描碎片比较内，Gallus表现出最高的局部乳突密度，而Columba把最大比例的壳体体积分配给由单个乳突knobs生长出的晶体单元。Anas在晶体单元占比上居中、在密度上则更接近Columba。这两个指标并没有压缩成一条单调轴线，但合起来说明，在考虑更晚出现的壳体特征之前，乳突层层面的对比已经可以被检测到。由于这一层是最早与蛋壳力学和基质调控相连的结构层级，我们随后进一步提出一个更窄的问题：这种已观察到的对比究竟反映了工具箱的整体替换，还是一个大体共享系统内部的差异性使用？", False, False),
])
...
p_sprot_focus_cn = spara([
    ("Gallus特异集合同时富集于蛋白N-糖基化（BP；图S5），因此比较重点从“有哪些蛋白”转向“共享蛋白以何种化学状态被使用”。保留下来的共享核心由此成为真正的分子背景，而共享蛋白上的糖基化则成为解释乳突层分化及后续壳体行为的最近端候选层。", [18]),
    ("从当前数据看，我们检测到的大多数反复出现的蛋壳基质蛋白与前人报道总体一致，说明整体蛋壳基质背景与既有研究相吻合；同时，这套数据也扩展了进入比较背景的基质蛋白范围。", [1, 2, 4, 10, 19, 21, 29]),
])

head("OVAL糖基化给出最清晰的跨物种分子对比")

p_s2a_cn = spara([
    ("完整糖肽分析显示，三物种在采样深度上存在差异，但仍共享一个稳定的比较核心（图2A至D）。在cluster层面，三物种共有25个共享cluster，而最大的额外两两重叠来自Anas和Columba的64个cluster；Gallus几乎没有形成明显的物种私有cluster空间（图2A）。", []),
    ("同样的模式也出现在清单数量上：Anas检测到321个糖蛋白、547个糖基化位点和197种糖链组成；Columba分别为192、257和162；Gallus分别为55、88和105（图2B）。尽管覆盖深度不同，共享核心的Jensen-Shannon相似性仍集中在0.33到0.40之间，其中Anas-Columba配对最高。这说明三物种的差异发生在一个仍可比较的糖蛋白背景之内，而不是彼此割裂的三个化学空间（图2C）。", []),
...
add_main_figure_legend(
    ...
    [
    ("(A) 糖蛋白组数据中的物种分区cluster数量。(B) 各物种检测到的糖蛋白、糖基化位点和糖链组成数量。(C) 物种间共享核心的Jensen-Shannon相似性。(D) 物种水平糖链类别分布（High Mannose、Pauci-mannose、Hybrid、Complex-Plain、Complex-Fucosylated、Complex-Sialylated和Other）。(E) 直系同源-糖链弦图，连接Gallus、Anas和Columba蛋壳糖蛋白与优势糖链类别，并高亮后续分析保留的基质蛋白。", False, False),
    ],
)
...
p_s2b_cn = mixed([
    ...
    ("G. gallus", False, True),
    ("为参考，只有当非参考候选的平均E值低于1 × 10⁻⁵且序列一致性满足最终可比性阈值时，才予以保留。这一过滤将后续比较限制在高置信度直系同源范围内。在这一更严格的映射下，OC17仅在Gallus中显示糖基化，而OC116、TRFE和OVAL在三物种中都保留了糖基化信号，可作为共享锚点。其中，OVAL表现出最清晰的跨物种糖链差异，因此成为后续结构分析的优先对象。", False, False),
])
cite(p_s2b_cn, [])

p_s2c_cn = spara([
    ("整合蛋白和糖链丰度后，OVAL进一步被识别为与跨物种蛋壳差异最一致的共享蛋白（图3A至C）。在全数据集中，Gallus的蛋白-糖链耦合较弱，而Anas和Columba则持续为正，说明不同谱系中糖基化随蛋白输出变化的方式并不相同。", []),
    ("在被高亮的蛋壳基质蛋白中，OVAL在三物种中都保持高丰度，但糖链负荷差异显著：Gallus相对较低，Anas更强，Columba最强。OC116和TRFE仍然是有信息量的共享蛋白，但都不像OVAL那样稳定地把总体蛋白丰度与糖链输出区分开。", []),
    ("随后，两两富集图说明了为什么OVAL始终是最清晰的区分指标（图3D至F）。在Gallus对Anas和Gallus对Columba的比较平面中，OVAL都落在糖链偏移更明显的一侧，因此其糖链变化并不是简单镜像蛋白丰度变化，而是快于、甚至部分逆于相应的蛋白丰度偏移。在Anas对Columba的平面中，OVAL再次偏离简单的蛋白-糖链等价关系，并把同样的排序延续到Gallus之外的比较中。完整糖肽鉴定进一步把OVAL放入一条连贯的跨物种序列：Gallus携带紧凑的高甘露糖型糖链，Anas富集中性复合/杂合型糖链，Columba则携带更延伸的唾液酸化复合/杂合型糖链。综上，图3A至F将OVAL界定为糖基化变化与表型关系最强的共享蛋白。", []),
])
...
add_main_figure_legend(
    ...
    [
        ("(A至C) Gallus、Anas和Columba物种内蛋白丰度与糖链丰度log2转换后的proteotype coevolution图；插图给出Spearman ρ和双侧p值，高亮标注保留的基质蛋白（OVAL、OC116、TRFE和OC17）。(D至F) Gallus对Columba、Gallus对Anas和Anas对Columba的两两二维糖链-蛋白富集图。虚线对角线表示蛋白和糖链变化幅度相等；向糖链富集侧偏移表示糖链变化超过蛋白丰度变化。", False, False),
    ],
)
...
p_s3a_cn = spara([
    ...
    ("代表性重建构象和物种特异性表面图显示，优势糖链在同一折叠蛋白骨架上占据不同空间包络（图4A和B）。在A图中，编号1和2分别标记糖链端到端向量的两端，3标记糖链质心，4标记蛋白Cα质心，半透明球体则把以3为中心的回转半径包络可视化出来。B图进一步把这种几何变化转成表面可及性：有颜色的区域是仍可接近Ca²⁺的相关区域，黑色区域则是这些区域被遮蔽后的状态。", []),
])

p_s3b_cn = spara([
    ("Columba首先通过占据最大的整体糖链包络与另外两种鸟分开，这一点体现在图4C更高的回转半径和图4E更长的端到端距离上。若把A图中1到2的向量看作糖链的‘展开长度’，那么Columba的这条向量更长，且以3为中心的球也更大；这就意味着它在同一蛋白骨架上覆盖了更大的空间范围，同时与4所代表的蛋白质心保持着更大的几何外扩。图4D中更近的局部骨架接近距离，以及图4F中更宽的糖链-蛋白距离分布，也都与这种更伸展但仍会回折到OVAL表面的构象一致。", []),
    ("Gallus则定义了相反的端点，其糖链更紧凑，对酸性界面的几何侵入最弱；Anas在回转半径和整体糖链-蛋白间距上更接近Gallus，但在端到端跨度和最小骨架接近距离上又与另外两者分开。因此，图4C至F把糖链类别推进转换成了一种遮蔽几何：A图里球越小、1-2跨度越短且更贴近4，说明糖链越紧凑；球越大、1-2跨度越长，说明糖链越延展并更容易参与表面遮蔽。", []),
])
p_s3c_cn = spara([
    ...
    ("界面遮蔽从Gallus到Anas再到Columba逐步增强，而热点表面积、热点比例和净可及Ca²⁺热点都保留了同样的排序。综合这些面板可以看出，在早期矿化阶段，共享的酸性OVAL表面被逐步遮蔽。", []),
])

p_s3d_cn = spara([
    ("界面遮蔽从Gallus到Anas再到Columba逐步增强，而热点表面积、热点比例和净可及Ca²⁺热点都保留了同样的排序。综合这些面板可以看出，在早期矿化阶段，共享的酸性OVAL表面被逐步遮蔽。", []),
    ("因此，Gallus保留了最强的推定Ca²⁺捕获能力，也最符合既往所提到的、矿化起始时OVAL较易进入Ca²⁺响应式开放状态的情形。Anas和Columba则从不同结构背景转向较低可及性一侧。相同的排序也对应了表型顺序：Gallus同时具有最致密的乳突场和最高的局部出壳抗性，而Anas和Columba则收敛到较低抗性一侧。因此，图4A至N把糖链依赖的分离、糖链几何、界面遮蔽以及共享基质蛋白上的Ca²⁺相关可及性连接在一起。", [4, 11, 29]),
])
...
mixed([
    ("F_max与τ_max之间的差异澄清了Anas的结果。它更高的原始接触力主要由更大的壳厚驱动（0.35 mm，而Columba为0.19 mm），而不代表更优越的单位面积材料抗性。相反，", False, False),
    ("G. gallus", False, True),
...
mixed([
    ("如果只看整壳层面的破裂力，Anas可能会因为壳更厚而显得在力学上优于Gallus，尽管它并不具备同样的高密度乳突状态。相反，τ_max聚焦于基于micro-CT重建的乳突界面局部出壳抗性，从而消除了这种歧义。它表明，高密度Gallus状态仍然保持独立，而Anas和Columba则在较低抗性上收敛。这一功能读数保留了前文已经显现出的同一不对称性，并把与糖基化相关的差异连接到三种模型物种的局部破壳力学上。", False, False),
])
...
p_disc_regulator_cn = spara([
    ...
    ("Anas在这一设计中尤其有信息量：它在发育上保留了广义早成状态，但 OVAL 糖链状态和可及性谱型转向中间型，且其 τ_max 结果与Columba而非Gallus收敛。由此，这一比较在保持出壳界面可比的前提下，覆盖了生态—发育连续空间中被有意拉开的区域。", [3, 15, 22, 23]),
], before=0, after=120)
...
p_disc_mam2_cn = spara([
    ...
    ("更早的Gallus研究为 OVAL 建立了糖基化位点基础，并鉴定 OC116 的糖基化 Asn。当前数据进一步解析了进入结构建模的 OVAL 直系同源 sequon 上的主导糖链类别（Gallus N293；Anas与 Columba N97）。相较既往位点检测研究，本研究的扩展在于跨物种比较广度与糖链类别解释深度，而非仅延长单物种位点清单。OVAL 的价值不在于其“独特”，而在于它在保持跨物种可比性的同时保留了糖链类别层面的可解释化学分化。", [8, 18, 21]),
])

p_disc_other_cn = spara([
    ("非OVAL信号仍然重要。OC116和TRFE仍是有信息量的共享蛋白，而OC17仅在Gallus中显示糖基化，因此可能代表一种更具谱系限制性的成矿程序。", [10, 19, 29]),
    ("前人的蛋壳研究已经分别赋予OC17、OC116和ovotransferrin相关组分明确的功能意义，而本研究并不是推翻这些认识。相反，我们的数据表明，这些蛋白更稳定地构成了生物学背景，而不是跨物种最尖锐的区分层。", [10, 19, 29]),
    ("对OC116尤其如此：更早的生化工作已经确认Gallus蛋壳基质OC116存在糖基化Asn，而近期的鸟类古蛋白组学工作又进一步显示，OC116是鸟类蛋壳蛋白中序列变异性最高的一类分子之一，并且在种内层面也具有显著变异，而不是一个始终稳定的物种标记。放在这一背景下，我们当前的比较结果就更容易理解：OC116保持糖基化并不等于它一定会像OVAL那样恢复出稳定的结构—表型排序。", [21, 66]),
...
p_disc_oval_cn = spara([
    ...
    ("Gallus的紧凑糖链使关键酸性 OVAL 表面保持相对暴露；Columba更长且电负性更强的糖链则在空间与静电两层面降低 Ca²⁺ 接近；Anas再次处于中间状态。", []),
...
p_disc_mech_cn = spara([
    ...
    ("Anas最清楚地体现了这种分离：更厚壳体抬高了 F_max，却未重建Gallus所呈现的高 τ_max 状态。", []),
])
...
p_disc_function_cn = para(
    "综上，在这组数据中，这一比较收束到一个Gallus式蛋壳状态。Gallus同时具有最致密的乳突场、受遮蔽最少的OVAL钙相关表面，以及内向外加载下最高的局部出壳抗性。这一模式支持一个更广的推论：在被重复使用的基质蛋白上，化学特异性的状态可能比单纯的蛋白组周转更清楚地组织矿化表型。",
    bold=False, size=11, before=0, after=120
)
cite(p_disc_function_cn, [67, 73, 74])

p_disc_selection_cn = para(
    "Anas和Columba仍然关键，因为它们共同定义了这一Gallus式状态在壳体结构与生态—发育位置上的边界。Anas将更大壳厚、中间型 OVAL 可及性与低 τ_max 结合在一起，表明仅凭厚度不能重建Gallus式状态。Columba在更薄壳体和不同乳突背景下与Anas在低 τ_max 上收敛。综合这些对照，Gallus在当前采样设计空间中成为连接糖链依赖基质行为与蛋壳性能的参考状态；在这一框架下，OVAL 糖链状态是使该状态获得力学可解释性的最直接分子层。",
    bold=False, size=11, before=0, after=120
)
...
p_disc_future_cn = para(
    "本研究范围仍有边界。我们分析的是主导糖型而非体内完整糖链系综；同时在平均蛋壳尺度上把各物种视为机械均一，并在 APBS 框架下使用约束尚不充分的子宫离子条件。就近期湿实验验证而言，一个现实限制是：在当前技术条件下，要在同一实验框架内同时实现物种匹配的同源基质蛋白特异表达以及特定位点定义糖链结构的精准修饰，在实践层面仍具挑战。与此同时，生物矿化本身是一个大规模耦合反应系统，也会限制单一实验对因果关系的直接解析。下一步关键检验仍包括：定义糖型的矿化实验、在Gallus中直接操控 OVAL 糖基化，以及对同一内向外力学对比进行位点分辨验证，并需配合方法学上的进一步发展以提升因果分辨率。这些实验将澄清 OVAL 糖链状态是直接参与壳体矿化，还是以异常高保真度标记Gallus式高抗性状态。",
    bold=False, size=11, before=0, after=120
)

p_disc_close_cn = para(
    "总之，本研究在三种鸟类蛋壳中把乳突层组织、糖蛋白状态、表面可及性与局部出壳力学连接在同一框架下。Gallus定义了这条轴线的高抗性端点，表现为致密乳突组织、紧凑 OVAL 糖链、更高 Ca²⁺ 相关表面暴露和乳突界面最高局部抗性。随着其他高丰度蛋壳基质蛋白获得可比糖型归属，这一框架可继续扩展。跨形态测量、糖蛋白组学、结构与力学四层证据，OVAL 糖链状态始终是与Gallus式蛋壳状态最一致的分子特征。",
    bold=False, size=11, before=0, after=120
)
...
mixed([
    ("采用既有的EDTA脱矿化方案提取蛋壳乳突层（EML）基质蛋白。Gallus蛋以去离子水漂洗后置入无菌密封袋。每个物种保留1枚蛋用于显微CT分析，其余蛋壳用于基质蛋白提取。剩余6枚Gallus蛋和6枚Anas蛋分别合并为3个两蛋提取单元，18枚Columba蛋合并为3个六蛋提取单元；这些合并提取单元同时作为shotgun proteomics和完整糖肽分析的匹配生物学样本。对 ", False, False),
    ("G.\u00a0gallus", False, True),
    (" 和 ", False, False),
    ("A.\u00a0platyrhynchos", False, True),
...
para(
    "用于shotgun proteomics的蛋壳基质提取样本与显微CT取样后的样本集对应：每个物种均为3个合并生物学重复，其中Gallus和Anas每个重复由2枚蛋组成，Columba每个重复由6枚蛋组成。蛋白质以裂解液（1% SDS、1%蛋白酶抑制剂混合物）重悬，冰上超声裂解后在4°C、12,000 × g离心10 min澄清，并以BCA法定量。蛋白质经预冷丙酮沉淀（5倍体积，−20°C，2 h）、丙酮洗涤两次后重溶于200 mM TEAB。二硫键以5 mM DTT在56°C还原30 min，再以11 mM碘乙酰胺在室温避光烷基化15 min。随后按酶:底物质量比1:50加入测序级胰蛋白酶过夜消化，并以Strata X SPE柱脱盐。LC-MS/MS分析前未进行离线HPLC分级；每次proteome采集上样20 μg肽段。"
)
