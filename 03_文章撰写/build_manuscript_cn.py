...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳是一个局部失效事件：破壳齿只作用于蛋壳内表面，而不是把力量分散到整个壳面。", [16, 86]),
    ...
    ("乳突层是最直接的切入点，因为它既是最早具有力学后果的蛋壳层，也是基质引导方解石生长的起点。", [1, 4, 20, 28, 30, 34, 57]),
])
...
p_intro2 = spara([
    ...
    ("这一缺口在糖基化层面最为明显。后修饰位点已经被记录，但糖链在组成、大小和电荷上变化更大，能够在同一蛋白支架上产生不同分子状态。", [17, 18, 21, 49, 50, 80]),
