...
p_s2a_cn = mixed([
    ("完整糖肽为鸟类蛋壳基质蛋白上的具体糖型提供了直接的跨物种视角。图2首先概括了糖蛋白组学数据的整体搜索空间，显示三物种在cluster数量、糖蛋白覆盖、糖基化位点数和糖链组成数上存在差异，但仍保留可比较的共享核心信号（图2A至C）。糖链类别组成进一步显示，高甘露糖型和岩藻糖化复合/杂合型构成跨物种广泛背景，而唾液酸化复合/杂合型及其他更延伸类别更明显地参与谱系差异（图2D）。", False, False),
...
add_main_figure_legend(
    ...
    "共享核心糖蛋白组结构与糖链类别部署。",
    [
        ("(A) 糖蛋白组数据中的物种分区cluster数量。(B) 各物种检测到的糖蛋白、糖基化位点和糖链组成数量。(C) 三物种共享核心的Jensen-Shannon相似性。(D) 物种水平糖链类别分布，包括High-Mannose、Paucimannose/Truncated、Neutral Complex/Hybrid、Fucosylated Complex/Hybrid、Sialylated Complex/Hybrid和Other。(E) 直系同源-糖链弦图，连接鸡、鸭和鸽蛋壳糖蛋白与优势糖链类别，并高亮后续比较保留的基质蛋白候选。", False, False),
    ],
)
