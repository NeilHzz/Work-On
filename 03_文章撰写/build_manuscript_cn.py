...
p_s0b_cn = mixed([
    ...
    ("G. gallus", False, True),
    ("最低（0.3975 ± 0.0127）。简单地说，鸡表现出最致密的早期成矿格局，鸽把最大比例的壳体体积分配给由单个乳突knobs启动的晶体单元，而鸭在晶体单元占比上居中、在密度上则更接近鸽。这两个指标并没有收束成一条简单的单调排序，但它们一致说明乳突层在更晚的壳体特征出现之前就已经发生了可测量的分化。由于乳突层是蛋壳力学最早建立的结构层，而乳突层又受蛋壳基质蛋白调控，接下来的问题自然变成：这种跨物种形态差异究竟对应蛋壳基质工具箱的整体更替，还是共享系统内部的不同使用方式。", False, False),
])
...
p_s2b_cn = mixed([
    ...
    ("G. gallus", False, True),
    ("为参考序列，非参考物种候选同源物只有在E值 < 1 × 10⁻⁵，且满足最终序列可比性阈值时才被保留，从而把后续比较限制在高可信直系同源物范围内。这一步之所以必要，是因为后续结构解释要求我们比较真正同类的蛋白，而不是同一家族里功能可能并不等价的不同成员。在这一步筛选后，OC17只在鸡中检测到糖基化，而OC116、TRFE和OVAL则在三物种中均保留糖基化信号，因此构成了真正可比较的共享锚点。其中，OVAL呈现出最清晰的跨物种糖链差异，因此成为后续结构分析的优先目标。", False, False),
])
...
p_disc_mam1_cn = smixed([
    ([('这篇文章的主线很直接。三种鸟的破壳齿功能保持一致，因此最有信息量的差异只能转移到蛋壳本身。第一层真正与力学相关的差异出现在乳突层，而共享基质蛋白工具箱进一步把比较收束到糖基化，而不是整体蛋白替换。', False, False),
            ('在这条链中，OVAL糖链状态提供了从分子变化到表面可及性和局部出壳抗性的最清楚连接。', False, False)], [1, 2, 4, 15, 16, 17, 20, 21, 22, 23, 38, 39, 41, 42, 82, 86]),
    ([(' 乳突层之所以仍居于核心，是因为这是基质化学、晶体起始和蛋壳力学首次在同一个材料层级汇合的地方。这个顺序本身也很重要，因为全文并不是先假定某个分子最重要，而是沿着层级一步步收束到那个候选层。', False, False)], [1, 2, 20, 28, 30, 36, 38, 53, 54]),
])
...
p_disc_mam2_cn = spara([
    ...
    ("也就是说，OVAL糖链状态并不是取代整个基质解释框架，而是为这一解释框架提供了一个最容易被化学和结构方式读出的中心环节。", [20, 21, 42, 61, 63, 66, 74, 78, 80]),
])
