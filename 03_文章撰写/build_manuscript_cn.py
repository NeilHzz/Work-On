...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳是一个局部事件：破壳齿作用在蛋壳内侧的一小块区域，而不是一次性加载整个蛋壳。", [16, 86]),
    ("类似的辅助出壳结构在卵生羊膜动物中反复出现，因此具有生物学意义的出壳差异更可能来自蛋壳本身，而不是工具本身。", [16, 82, 83, 84, 85]),
    ("但目前我们仍不清楚：在出壳力学相近的前提下，不同物种如何在生态与发育背景中形成不同的蛋壳状态。", [15, 26, 39, 40, 41]),
    ("乳突层是关键切入点，因为它是蛋壳中最早对力学产生实质影响的层级，也是基质引导方解石生长启动的位置。机制问题由此直接落到：当破壳界面被固定后，乳突层上的哪些分子调控因子解释了跨物种的蛋壳状态差异？", [1, 2, 4, 16, 28, 57]),
])

# §2 — 已知工作与局限
p_intro2 = spara([
    ("蛋壳基质蛋白调控乳突层成矿、晶体生长和成熟蛋壳结构；OC17、OC116、TRFE 与 OVAL 等反复出现的因子构成了共享的成壳工具箱。", [1, 2, 4, 10, 19, 21, 29]),
    ("当前未解决的问题不是这些工具是否存在，而是它们在不同物种中如何被部署。", [1, 2, 4]),
    ("这一缺口在翻译后修饰层面最突出。磷酸化与糖基化位点都已有目录，但磷酸基侧链相对相似，而糖链在组成、大小与电荷上差异更大，能够在同一蛋白骨架上形成不同分子状态。", [17, 18, 21, 49, 50, 80]),
    ("因此，糖链类别是机制变量，而不仅是位点占有率特征。", [49, 50]),
    ("糖蛋白组学研究已表明，蛋壳基质蛋白存在不同的 N-糖基化状态，包括 OC116 的糖基化 Asn 和已定义的 OVAL 相关糖链组成；但多数分析仍局限于单一物种、单一区室或单一位点清单。", [7, 8, 18, 21, 47, 48]),
    ("因此，鸟类蛋壳比较仍很少在跨物种尺度上解析共享基质蛋白的匹配糖链状态，糖链层是否解释“相似工具箱为何产生不同蛋壳状态”这一问题仍然开放。", [2, 4, 18, 29, 66]),
])

p_intro_sig_cn = spara([
    ("糖基化可改变蛋白稳定性、分子识别与表面可及性；在其他体系中，糖链也可作为动态遮蔽层重塑可接近界面。", [42, 43, 44, 61, 63, 72, 78, 81]),
    ("分层蛋壳研究进一步表明，同一基质蛋白在不同壳层区室可对应不同 N-糖基化状态，这提示糖链状态可能重新分配功能，而不仅是装饰固定骨架。", [18]),
    ("因此，OVAL 是一个可检验的案例：其丰度高、与成矿相关，并与早期成壳阶段的 Ca²⁺响应构象行为有关。", [4, 11, 18, 29]),
    ("我们据此将比较锚定在保守的破壳齿界面上，检验共享基质蛋白上的糖链状态差异是否会在矿化起始时生成不同的 Ca²⁺可接近表面。", [4, 16, 18]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("当前缺失的关键步骤，是把糖链类别直接连到“共享基质背景上的表面呈现”；OVAL 提供了这座桥梁，因为其优势糖链类别可以从糖蛋白组学一路追踪到结构建模。", [4, 18, 29, 42]),
])
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ("鸽", False, False),
                ("。", False, False)], []),
    ([ ("我们建立了一套整合方案：结合显微CT形态测量、比较蛋白组学、完整糖肽质谱、Re-Glyco建模与静电分析，以及有限元模拟，在跨尺度层面联结蛋壳结构、糖链状态与出壳力学。", False, False)], []),
    ([ ("分析结果显示：在更广的基质蛋白工具箱出现分离之前，物种首先在乳突层组织上分开；而 OVAL 糖链状态是连接分子状态、表面可及性与局部出壳抗性的最强信号。", False, False)], []),
    ([ ("这些结果共同建立了一个框架，用于在鸟类中把糖链状态变化与蛋壳组织及内向外失效行为连接起来。", False, False)], []),
])
...
p_sprot_go_cn = spara([
    ("GO富集和基因家族周转进一步提示了谱系特异的免疫与防御背景（图S5、S6和S9），但这些信号更多描述的是比较背景，而不是与乳突层组织最直接对应的层级。这些带有谱系偏向的信息仍然是重要的演化背景，但它们本身并没有指出把共享基质体系与乳突层分化及后续出壳相关力学连接起来的最近端层级。", []),
])
...
p_s2c_cn = spara([
    ...
    ("图3E至G中的两两富集图进一步说明了为什么OVAL是最清晰的候选。在鸡对鸭和鸡对鸽的比较平面中，OVAL都落在糖链变化相对更突出的区域，说明它的糖链偏移并不是简单跟随蛋白丰度一起移动，而是比蛋白丰度变化更快，甚至与之部分脱耦。在鸭对鸽的比较中，OVAL仍然偏离蛋白-糖链完全一致的对角关系，从而把同一排序延续到鸡之外的比较中。完整糖肽鉴定因此把 OVAL 放入一条连贯的跨物种糖型序列：鸡以紧凑高甘露糖型为主，鸭转向中性复合/杂合型，鸽则进一步转向更延伸的唾液酸化复合/杂合型。综合来看，图3B至G将OVAL界定为糖基化变化与表型联系最稳定的共享蛋白。", []),
])
...
p_s3a_cn = spara([
    ...
    ("但一旦去除糖链，这种分离中的大部分就会明显收敛，三物种蛋白骨架彼此更接近。因此，最初被恢复出的结构差异并不是一般性的序列差异，而是糖链把矿化起始时暴露给离子环境的酸性表面重新组织了出来（图4A至C；图S10）。", []),
])
...
p_disc_mam2_cn = spara([
    ...
    ("前人的鸡来源研究已经为OVAL建立了糖基化位点基础，也确认了OC116中存在糖基化Asn；本研究则进一步解析了被带入结构建模的对应OVAL直系同源测序子上的优势糖型（鸡 N293；鸭与鸽 N97）。相较于既往以位点目录为主的工作，这里的扩展首先体现在跨物种 breadth，其次体现在把优势糖型直接推进到结构和力学解释之中。因此，OVAL的价值不在于其独特，而在于它在保持跨物种可比性的同时，仍保留了具体糖链类别层面的可解释化学分化。", [8, 18, 21]),
])
...
p_disc_future_cn = para(
    "当前结论仍有明确边界。我们的结构分析采用的是优势糖型，而不是体内全部糖型的完整分布；有限元模型也把各物种蛋壳近似为平均尺度上的机械均一材料，APBS所需的子宫液离子环境同样仍未被充分约束。在当前技术条件下，要在同一体系中同时实现跨物种同源基质蛋白的匹配表达与特定位点预定义糖链结构的精准安装，在实践层面仍具挑战。与此同时，生物矿化本身是一个大规模耦合反应系统，这也会限制单一实验结果的直接因果外推。下一步最关键的检验应当是定义糖型的矿化实验、在鸡中直接操控OVAL糖基化，以及对同一内向外力学对比进行位点分辨验证并继续发展提高因果解析度的方法。这些实验将决定，这里识别出的OVAL糖链状态究竟直接参与成壳矿化，还是以异常稳定的方式标记了鸡式高抗性状态。",
    bold=False, size=11, before=0, after=120
)
...
p_m_bio = mixed([
    ...
    ("（绿头鸭）蛋来源于山东某蛋鸭养殖场。所有蛋壳样品于处理前在4\u00b0C下保存；统一的新鲜采集与冷藏条件用于尽量减少产蛋后处理带来的额外变异。", False, False),
])
