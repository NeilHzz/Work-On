...
p_disc_future_cn = para(
    "本研究范围仍有边界。我们分析的是主导糖型而非体内完整糖链系综；同时在平均蛋壳尺度上把各物种视为机械均一，并在 APBS 框架下使用约束尚不充分的子宫离子条件。就近期湿实验验证而言，一个现实限制是：在当前技术条件下，要在同一实验框架内同时实现物种匹配的同源基质蛋白特异表达以及特定位点定义糖链结构的精准修饰，仍具有较高技术难度。与此同时，生物矿化本身是一个大规模耦合反应系统，也会限制单一实验对因果关系的直接解析。下一步关键检验仍包括：定义糖型的矿化实验、在Gallus中直接操控 OVAL 糖基化，以及对同一内向外力学对比进行位点分辨验证，并需配合方法学上的进一步发展以提升因果分辨率。这些实验将澄清 OVAL 糖链状态是直接参与壳体矿化，还是以异常高保真度标记Gallus式高抗性状态。",
    bold=False, size=11, before=0, after=120
)
