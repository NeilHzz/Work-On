...
OUT = str(Path(__file__).with_name("manuscript260518_cn.docx"))
...
para(
    ...
    "在共享蛋白中，OVAL表现出清晰的糖链状态对比：鸡以高甘露糖型为主，鸭转向中性复合/杂合型，鸽进一步转向唾液酸化复合/杂合型。"
    "结构建模表明，这些状态使Ca²⁺相关表面可及性沿鸡到鸽逐步下降；有限元分析在局部出壳抗性中恢复了鸡与较低抗性鸭/鸽之间的同一对比格局。"
...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳首先面对的是一个很简单的力学问题：胚胎必须把力集中到一个局部破壳位点。", [16]),
    ("在鸟类中，这个位点由短暂存在的破壳齿定义，它从壳内侧顶压蛋壳并触发出壳。", [86]),
    ("如果破壳齿的功能在不同鸟类之间总体保守，那么真正有意义的出壳差异就更可能来自蛋壳，而不是来自破壳工具本身。", []),
    ("类似的辅助出壳结构也见于其他卵生羊膜动物，包括单孔类、蜥脚类胚胎和壁虎，这进一步说明，在考察蛋壳分化时，把破壳界面先固定下来是有意义的。", [82, 83, 84, 85]),
    ("鸟类蛋壳会随孵化环境和生活史策略变化，而不是在鸟类中呈现一种统一方案。", [15, 39, 40, 41]),
    ("不同栖息地会改变卵所面对的失水、气体交换和微生物暴露压力，因此陆生与强水域关联鸟类并不需要构建完全相同的壳体屏障和孔道体系。", [39, 40, 41]),
    ("类似的蛋壳微结构与孵化力学分化在非当前焦点物种之外也有报道，这说明壳体结构在不同孵化制度下始终具有较强的演化可塑性。", [53, 54, 68]),
    ("与此同时，早成和晚成也提供了另一条比较轴线：雏鸟独立程度和亲代缓冲方式不同，而比较研究已经把这种差异与壳表角质层配置以及蛋壳抗菌表现联系起来。", [15, 26]),
    ("乳突层是最适合开始分析的层级，因为它是最先影响力学的蛋壳层，其乳突结节标记了方解石在蛋壳基质蛋白调控下开始生长的位置。", [1, 4, 20, 28]),
    ("强烈成壳鸟类中的钙转运生理也对这一早期成矿窗口构成约束。", [34, 57]),
    ("因此，蛋壳让我们能够在同一个结构里同时解读基质化学、微结构和与出壳相关的力学。", [1, 16, 30]),
    ("由于后续壳层会继承这一早期成矿背景，乳突层也就是局部分子差异最可能被放大为成熟蛋壳行为差异的位置。", [1, 4, 28]),
])

p_s1b_cn = spara([
    ("因此，乳突层应当成为首先被检验的表型，因为它是最早可能把结构差异转成力学差异的蛋壳层级。", [1, 16, 28]),
    ("而一旦比较从保守的破壳工具转移到这一与力学直接相关的蛋壳层，下一步就要问：究竟是哪类分子在组织这一层，以及这些分子在不同物种中以什么状态发挥作用。", [1, 2, 4]),
    ("因此，这个比较不再只是描述差异，而是转向一个机制问题：当界面被固定后，同一套成壳系统究竟怎样在不同物种中被调到不同状态。", [1, 2, 4]),
])

# §2 — 已知工作与局限
p_intro2 = spara([
    ("蛋壳基质蛋白之所以是最直接的候选层，是因为它们调控乳突层成矿、晶体生长以及成熟蛋壳的结构组织。", [1, 2, 4]),
    ("其中，OC17、OC116、TRFE 和 OVAL 都是反复被讨论的代表性蛋白。", [10, 19, 21, 29]),
    ("也正因为这些蛋白在鸟类蛋壳研究中反复出现，它们构成了检验“共享成矿工具箱如何在不同壳体背景中被重复使用”的天然比较锚点。", [1, 2, 4]),
    ("但这些蛋白并不只通过丰度发挥作用。已有蛋壳糖蛋白组学研究表明，同一种蛋壳基质蛋白可处于不同的N-糖基化状态，这意味着糖基化程度差异可在不整体替换蛋白的情况下改变生物学功能。", [18]),
    ("平行开展的鸟类卵相关研究还在蛋清、卵索和卵黄膜层面描绘了N-糖基化蛋白图谱，并且孵化阶段分析也显示鸡卵中的糖基化状态会发生变化。", [7, 47, 48]),
    ("但鸟类蛋壳研究很少在跨物种框架中解析共享基质蛋白具体携带哪些N-糖链形式。", []),
    ("这一缺失限制了比较解释：以往蛋壳比较研究大多根据蛋白身份或丰度推测机制，却无法直接观察这些共享蛋白携带的具体糖链形式。", []),
    ("因此，真正缺失的一层并不是基质蛋白是否重要，而是共享基质蛋白上的糖基化能否解释为什么相似的蛋白工具箱会产生不同的蛋壳结构。", [2, 4, 18]),
])

p_intro_sig_cn = spara([
    ("这一缺口之所以重要，是因为糖基化可影响多种蛋白性质，包括稳定性、分子间识别、表面暴露和构象调控；其中，折叠状态在这里尤其重要。", [42, 43, 72, 78]),
    ("其他糖蛋白系统的研究也表明，糖链并不是被动附着的体积负担，而可以作为动态遮蔽层去重塑表面暴露与识别行为。", [42, 43, 61, 63]),
    ("既往矿化相关工作提示，OVAL 在早期成壳阶段可能进入 Ca²⁺ 响应的构象状态，因此糖链依赖的表面可及性就成为一个可以检验的机制变量。", [4, 29]),
    ("相关系综研究进一步说明，在共享蛋白骨架上，糖链差异本身就可能改变构象采样与表面可及性。", [61, 63, 81]),
    ("基于这一逻辑，我们试图将糖链状态检测与Ca²⁺相关可及性联系起来：如果糖链差异会重塑同一共享基质蛋白呈现折叠表面的方式，那么结构建模就应揭示矿化起始时呈现给Ca²⁺的表面差异。", [11, 42, 43]),
    ("如果这种结构差异具有生物学意义，那么在出壳相关的力学终点上也应当仍可见，即破壳齿样加载下乳突界面的局部抗性。", [16, 37, 69]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("因此，我们将核心问题进一步收窄：当比较锚定到保守的破壳齿界面后，共享基质蛋白上的糖链状态能否帮助解释，同一套成壳工具箱为何会与不同的Ca²⁺可接近状态相联系？", [18, 42]),
    ("OVAL提供了一个便于优先检验的案例，因为既往矿化相关工作已使其 Ca²⁺ 响应表面行为具有生物学意义，因此有理由进一步追问糖链状态是否参与塑造这一钙可接近背景。", [4, 29]),
    ("因此，全文的分析链条依次从保守的破壳工具走向蛋壳结构、基质调控、糖基化、Ca²⁺可接近表面状态、结构推断和有限元验证。", []),
    ("我们的目的并非把三种鸟强行压缩成一条过度简单的单一轴线，而是检验在一个保守界面上，是否仍能辨认出一条从壳体结构到分子状态再到局部功能的连贯路径。", []),
])
...
p_intro4 = smixed([
    ([ ("本文比较了", False, False),
            ...
            ([ ("在当前数据中，这条链在OVAL上最为清晰，其糖链状态与乳突密度、Ca²⁺相关表面可及性和局部出壳抗性一致变化。", False, False)], []),
            ([ ("因此，OVAL并不是预先指定的偏好候选，而是在完整比较过程中变得最为清晰可读的共享蛋白。", False, False)], [18]),
            ([ ("因此，这里的引言也以与全文相同的因果顺序引向结果：保守的破壳齿功能、蛋壳焦点、乳突层组织、基质蛋白、糖基化、结构推断和有限元验证。", False, False)], []),
])
...
p_sprot_bg_cn = spara([
    ("正交组分析显示，三物种蛋壳基质蛋白组中仍存在一个大的共享核心，同时叠加了两两共享和谱系限制性成分（图S3）。这一格局说明，这些蛋壳差异并不是由蛋壳基质蛋白工具箱的整体替换驱动的。", []),
    ("从整体格局看，这一层蛋壳基质蛋白组仍较好保留了系统发育所界定的比较背景。因此，真正需要解释的重点不再是“有哪些蛋白”，而是“共享蛋白是怎样被使用的”。", []),
    ("因此，这个共享核心不是可以忽略的剩余项，而是后续所有候选机制都必须放回去理解的主要分子背景。", []),
])

p_sprot_go_cn = spara([
    ("GO富集与基因家族周转进一步表明，三条谱系在免疫与防御相关背景上确有稳定分化（图S5和S6），但这些背景差异本身并不能解释乳突层的结构表型差异。", []),
    ("因此，它们更适合作为演化背景来理解，而不是直接给出连接壳体结构与出壳相关力学的最近端解释层。", []),
])

p_sprot_focus_cn = spara([
    ("与此同时，鸡特异集合在蛋白N-连接糖基化（BP；图S5）上显著富集，说明最有信息量的差异不仅位于蛋白组背景，也位于翻译后状态。", []),
    ("由于糖基化可以改变同一基质蛋白的功能，这一结果把分析从广义蛋白组背景收窄到比较糖蛋白组学，具体而言，就是比较三物种共享蛋白上的N-连接糖基化如何不同。", [18]),
    ("因此，这一比较不再只问有哪些基质蛋白存在，而是问这些蛋白上的哪些化学状态在各条谱系中被优先部署。", []),
])
...
p_s2c_cn = spara([
    ...
    ("更突出的是，在重点蛋壳基质蛋白中，高丰度并不等于高糖链输出。OVAL在三物种中都保持高丰度，但其糖链负荷变化明显：鸡相对较低，鸭更强，鸽最强。OVAL因此既保持了跨物种可比性，又不受简单丰度匹配限制。", []),
    ("相比之下，OC116和TRFE仍然是有信息量的共享蛋白，但在同一分析空间中，并不像OVAL那样稳定地区分总体蛋白丰度与糖链输出。", []),
    ("图3E-G中的两两富集图进一步强化了这一点。完整糖肽鉴定显示，OVAL构成了最清晰的跨物种对比：鸡以紧凑高甘露糖型为主，鸭转向中性复合/杂合型，鸽则进一步转向更延伸的唾液酸化复合/杂合型。因此，图3B-G将OVAL界定为糖基化变化与表型联系最稳定的共享蛋白。", []),
])

p_s2d_cn = spara([
    ("由于这些OVAL糖链类别在大小和电荷分布上差异显著，这一比较更适合把OVAL表面可及性而非单纯OVAL丰度视为一个可解释的比较变量。", []),
    ("因此，接下来的结构问题是糖链状态如何在早期成矿过程中改变暴露且具有化学活性的OVAL表面；这一特征可以通过结构系综和静电计算直接检验，而不必只从丰度矩阵推断。", []),
    ("真正相关的变量并不只是OVAL有多少，而是被不同糖链装饰后，其酸性界面还有多少保持化学可及。", []),
    ("直系同源控制、丰度解耦和糖链类别推进三方面证据共同将OVAL保留为唯一同时保持可比、化学特异且可进行结构追踪的共享候选。", []),
])

head("OVAL糖链状态重塑表面可及性")

p_s3a_cn = spara([
    ("OVAL随后被作为最强共享候选，用于检验其糖链类别如何改变生物物理可及性。既往体外研究已表明，OVAL能够参与早期矿化相关事件，因此糖链状态构成了一个在生物学上合理的控制层。为此，我们重建了主导的糖基化OVAL构象，并为每个物种配对构建去糖基化参考，以检验三物种差异是否主要来自糖链依赖的表面行为，而非仅来自蛋白骨架序列本身。", [4, 11]),
    ("在这一共享结构框架内，首先要判断的是，单独改变糖基化状态是否就足以把各物种与其配对的去糖参考分开。图4A中的Ca²⁺热点，指的是表面Asp/Glu酸性位点中电势足够负、因而有利于局部钙离子接近的位置；图4B显示这些带羧酸基的表面还有多少保持物理暴露；图4C则将同一变化可视化为整表面电势图。三物种中，糖基化都会使这些读数偏离配对的去糖基化参考，而一旦去除糖链，三物种蛋白骨架之间的大部分分离就会明显收敛。这说明大部分跨物种差异是由糖基化引入，而不是由蛋白支架本身造成（图S7）。", []),
])

p_s3b_cn = spara([
    ("这种由糖链引入的分离随后需要一个几何解释。图4D中的回转半径反映糖链占据空间的总体尺度，图4E中的端到端距离反映糖链延伸长度，图4F中的糖链-蛋白距离反映糖链整体离蛋白表面的远近，图4G中的最小糖链-骨架距离则反映糖链是否回折并贴近蛋白表面。", []),
    ("综合来看，鸽的糖链占据了最大的构象空间，同时也最贴近蛋白表面，说明它们更长，但也更贴表；鸡的糖链最紧凑、与蛋白接触最弱；鸭则处于中间状态。因此，三物种的差异不仅在于糖链类别，还在于糖链层以何种空间方式覆盖在同一个OVAL表面之上（图4D-G）。", []),
])
p_s3c_cn = spara([
    ("既然几何已经分化，下一步就要判断这些差异是否改变了糖链层遮蔽共享酸性OVAL界面的程度。图4H量化相关酸性界面被糖链遮蔽的程度；图4I给出热点残基的溶剂可及表面积，也就是这些酸性位点还有多少仍向溶剂开放、因而也可能向离子开放；图4J记录候选酸性残基中仍保留为热点的比例；图4K统计在电势上仍有利于Ca²⁺接近、且在空间上也保持可及的净热点数量。", []),
    ("在这一组指标中，表面遮蔽从鸡到鸭到鸽逐步增强，而暴露热点面积、热点比例和净可及热点数量则沿相反方向逐步下降。由此，结果的含义已经变得具体：三物种的差异并不是‘带了不同的糖链’这么笼统，而是这些糖链在多大程度上覆盖了同一片原本能够与局部Ca²⁺发生接触的共享酸性界面（图4H-K）。", []),
])

p_s3d_cn = spara([
    ("当把可及性进一步拆分为暴露和遮蔽两个部分时，同样的跨物种对比格局仍然保留在整体界面层面。图4L把热点数量拆分为暴露成分和糖链遮蔽成分，图4M则对热点残基表面积做同样拆分，因此可以同时看到潜在与Ca²⁺相关的表面总量，以及其中在糖链覆盖后仍保留下来的部分。", []),
    ("在这一总括层面，鸡保留了最大的暴露份额，鸽把最大的份额转入了被遮蔽部分，鸭仍然位于两者之间。因此，三种主导OVAL糖型最终收束到同一个共享的物理对比上：鸡的高甘露糖型使酸性界面保持最开放，鸭的中性复合/杂合型使其部分覆盖，而鸽的唾液酸化复合/杂合型则造成最强的整体遮蔽。再糖基化和电势分析据此将这一跨物种对比解析为一条逐层递进的物理解释链：从糖链组成，到糖链几何，到界面遮蔽，再到共享基质蛋白上与Ca²⁺相关的表面可及性（图4L-M）。", []),
])
...
p_disc_mam1_cn = smixed([
    ([('结果界定出一条一致的比较顺序。由于三种鸟的破壳齿功能保持保守，最有信息量的差异只能在蛋壳中寻找。第一层与力学相关的差异出现在乳突层，而共享基质工具箱随后把比较聚焦到糖基化，而不是整体蛋白替换。', False, False),
            ('在这条链中，OVAL糖链状态为分子变化、表面可及性和局部出壳抗性之间提供了当前数据中最可解释的比较对应。', False, False)], [1, 16, 18]),
])

p_disc_regulator_cn = spara([
    ("这一比较设计之所以重要，是因为蛋首先服务于后代繁殖，而与蛋最直接相关的并不是某个二值标签，而是巢穴环境和雏鸟状态这两条连续轴线。", [15, 39]),
    ("巢穴环境与陆生到强水域依赖的梯度高度相关，雏鸟状态则从更早成到更晚成连续分布，因此两者都不适合被压缩为简单的二分法。", [3, 23]),
    ("前人的比较讨论往往把栖息环境关联或发育方式当作蛋壳生物学的类别标签来使用；本研究是在这些工作的基础上进一步把两者都作为连续轴线处理，并有意选取该共享空间中彼此分离的区域。", [3, 15, 23]),
    ("鸭正因此避免了这一比较被压缩成简单的“早成对晚成”对照：它保留了广义上的早成发育条件，但OVAL糖链状态和可及性谱型转向中间状态，τ_max结果也与鸽而非鸡收敛。", []),
    ("换句话说，这组三物种的价值不在于构成整齐类别，而在于它们在生态—发育连续空间中被有意拉开，从而尽量减少中间组合对比较解释的干扰。", [3, 22]),
], before=0, after=120)
...
p_disc_mam2_cn = spara([
    ...
    ("直系同源组更替、基因家族变化和糖蛋白网络分化仍然重要，但它们主要界定了比较背景，而不是最近端的解释层。", [18, 27]),
    ("OVAL糖链状态尤其有信息量，因为它跨物种共享、具有化学可解释性，并位于一个已被认为参与矿化的高丰度基质蛋白上。", [4, 18]),
    ("前人的工作已经把OVAL保留为高丰度蛋壳糖蛋白和潜在成矿候选，而更早的糖蛋白组研究也提示蛋壳基质蛋白可以处于不同的N-糖基化状态；本研究的推进在于，用直系同源约束下的跨物种比较识别出其中哪一种状态与表型对应最清楚。", [4, 18]),
    ("这一点在位点层面也同样重要。前人的鸡来源研究已经为OVAL建立了糖基化位点基础，也确认了OC116中存在糖基化Asn；而本研究是在这一基础上进一步解析被带入结构建模的对应OVAL直系同源测序子上的优势糖型（G. gallus N293；A. platyrhynchos与C. livia N97），而不只是停留在“存在糖基化”这一层。", [8, 18]),
    ("这一组合使OVAL的价值不在于其独特，而在于它在保持跨物种可比性的同时，仍携带可解释的化学分化。", []),
    ("它的可解释性也落实在具体糖链类别层面，而不只是修饰负荷的一般升高或降低。", []),
])

p_disc_other_cn = spara([
    ("非OVAL信号仍然重要。OC116和TRFE仍是有信息量的共享蛋白，而OC17仅在鸡中显示糖基化，因此可能代表一种更具谱系限制性的成矿程序。", []),
    ("前人的蛋壳研究已经分别赋予OC17、OC116和ovotransferrin相关组分明确的功能意义，而本研究并不是推翻这些认识；相反，我们的数据表明，这些蛋白更稳定地构成了生物学背景，而不是构成跨物种最尖锐的区分层。", [10, 19, 29]),
    ("对OC116尤其如此：更早的生化工作已经确认鸡蛋壳基质OC116存在糖基化Asn，而我们当前的比较进一步显示，保留糖基化本身还不足以像OVAL那样恢复出同样清晰的结构关联对比。", [21]),
    ("即便其中一个组分提供了当前最易检验的解释切入点，共享工具箱仍然是多组分的。", []),
    ("这些结果并不意味着一个蛋白就能解释整个蛋壳；相反，在当前数据中，OVAL提供了一条最可操作的比较路径，使我们能够合理推想基质修饰如何进一步传递到结构与功能层面。", []),
], before=0, after=120)

p_disc_oval_cn = spara([
    ("Re-Glyco与APBS分析为这一论证提供了结构桥梁。", []),
    ("鸡的紧凑糖链使关键酸性的OVAL表面保持相对暴露，而鸽更长且电负性更强的糖链则在空间和静电两个层面削弱了Ca²⁺的接近；鸭再次位于两者之间。", []),
    ("前人的体外和结构工作已经提示OVAL的构象与静电性质会影响矿化过程，但它们并没有在鸟类物种之间比较一组彼此匹配、且能分辨糖型的表面系综；就我们所知，这一步比较是本研究首次完成的。", [4, 11]),
    ("这样，糖链状态差异就从组成差别转变为可作物理解释的表面差异。", []),
    ("尽管这一结果并不建立矿化的直接因果机制，但它支持一种有边界的推论：同一基质蛋白上的不同糖链状态可能改变呈现给成矿环境的化学表面，并因此合理参与这里观察到的结构分化。", [42, 61]),
], before=0, after=120)

p_disc_mech_cn = spara([
    ("力学分析把同一模式延伸到个体层面的功能背景。", [16, 37, 69]),
    ("比起原始破裂力，τ_max更能捕捉乳突层级的力学差异。", [34, 69]),
    ("这一点也构成了与多数前人蛋壳力学研究的区别：以往工作更常强调破裂力、壳体强度或整壳质量指标，而这里真正要问的是，一个局部乳突界面指标是否会恢复与分子和结构比较相同的对比格局。", [34, 35, 69]),
    ("绝对失效载荷仍受壳厚和整壳几何影响，而τ_max更直接地读出出壳相关加载下乳突界面的局部抗性。尽管鸭和鸽的总体乳突几何不同，二者在τ_max上却收敛，这说明一旦缺少鸡那种高密度状态，仅靠更下游的形态变化并不能恢复同等级别的乳突界面抗性。", [37, 69]),
    ("因此，相比整壳受力，τ_max更适合作为与前述分子和形态结果并列解读的功能指标。", [34, 69]),
])
...
p_disc_discriminate_cn = spara([
    ...
    ("壳厚、体型和广义繁殖生态都可能提供背景差异，谱系历史无疑也重要。", [3, 14]),
    ("但基于厚度的解释无法说明τ_max差异，而弥散的谱系分化解释也无法说明为什么同一对比会在糖链类别、静电可及性和乳突层组织中反复出现。", [16, 37]),
    ("因此，在这组数据中反复出现的并不是单轴化的物种排序，而是糖链状态、表面遮蔽、乳突层组织和τ_max分组之间的重复对齐。", []),
    ("在这组三物种数据中，生态与系统发育设定了比较背景，而基质蛋白糖链状态则提供了当前恢复出的最近端、且具有化学可解释性的层级，而不只是一个被动背景描述。", [4, 18]),
], before=0, after=120)

p_disc_function_cn = para(
    "这项工作的更广泛意义首先体现在比较生物矿化的方法层面。共享基质蛋白上的化学特异性翻译后状态，在一个可处理的比较数据集中，有助于填补蛋壳结构与局部功能之间缺失的那一层。在生物矿化系统中，广义蛋白组周转往往比可能偏置晶体生长的具体化学状态更容易被检测到。类似逻辑能否超出鸟类蛋壳，还需要进一步检验，而不宜直接假定；毕竟许多矿化系统都依赖高丰度基质蛋白，而这些蛋白的翻译后状态可以在不整体替换底层蛋白库的情况下发生改变。在这个较窄的意义上，当前研究展示的是：在三物种蛋壳比较中，保守的蛋白工具箱可以通过糖链状态的再部署呈现不同的结构结果。",
    bold=False, size=11, before=0, after=120
)
cite(p_disc_function_cn, [67, 73, 74])

p_disc_selection_cn = para(
    "在这一三物种蛋壳数据集中，OVAL糖链状态构成了一个具有化学特异性的比较层级：它既能被映射到结构，也能被放到功能背景下检验。在富组学的生物矿化研究里，谱系差异往往比最稳定组织表型本身的分子特征更容易被检测到。在这组三物种比较中，相比广义蛋白组周转，糖链状态是更容易解释的一层。这种直系同源过滤后的设计之所以有价值，正在于它能在同一数据集中把“谱系周转”和“修饰状态变化”分开来看，尽管当前数据的物种范围仍然有限。同样的比较逻辑可以在其他高丰度基质蛋白中继续检验，前提是其修饰状态也能以类似置信度被解析出来。在其他矿化体系中，承担类似角色的层面也可能转而由复用基质蛋白上的硫酸化、磷酸化、蛋白水解加工或受调控的辅因子结合来承载。",
    bold=False, size=11, before=0, after=120
)
