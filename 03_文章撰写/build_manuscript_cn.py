...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳是一个局部化失效事件：力不是分散到整个壳面，而是通过破壳齿压向蛋壳内表面。", [16, 86]),
    ("类似的辅助出壳结构也见于其他卵生羊膜动物，这说明真正有生物学意义的出壳性能差异更可能位于蛋壳，而不是工具本身。", [16, 82, 83, 84, 85]),
    ("尽管如此，鸟类蛋壳仍随生态与发育状态而分化，因此鸟类实现出壳依赖的不是单一通用结构，而是多种蛋壳解决方案。", [15, 26, 39, 40, 41]),
    ("因此，乳突层成为最关键的切入点，因为它既是最早具有力学后果的蛋壳层，也是基质引导方解石生长的起点，局部分子差异可由此放大为成熟蛋壳行为。", [1, 4, 20, 28, 30, 34, 57]),
])

p_s1b_cn = spara([
    ("因此，真正的机制问题就变成：当破壳界面被固定后，究竟是哪些作用于乳突层的分子调控因素，解释了不同物种恢复出的不同蛋壳状态？", [1, 2, 4, 16, 28]),
])
...
p_intro2 = spara([
    ...
    ("这一缺口在糖基化层面最为突出：后修饰位点已被记录，但不同于化学上较受限的磷酸侧链，糖链在组成、大小和电荷上变化极大，因此能在同一蛋白支架上产生不同分子状态；决定机制的，不只是位点是否被占据，更是糖链类别本身。", [17, 18, 21, 49, 50, 80]),
    ("然而，现有比较研究很少解析共享基质蛋白在跨物种间携带的匹配糖链状态，因此仍不清楚这一糖链层是否解释了相似工具箱为何产生不同蛋壳状态。", [2, 4, 7, 18, 29, 47, 48, 66]),
])

p_intro_sig_cn = spara([
    ("糖基化会改变蛋白稳定性、分子识别和折叠表面的可及性；在其他系统中，糖链更像动态遮蔽层，可直接重塑可接近界面。", [42, 43, 44, 61, 63, 72, 78, 81]),
    ("分层蛋壳研究也表明，同一种基质蛋白在不同壳层区室之间可以处于不同的 N-糖基化状态，这意味着糖链状态可能重新分配功能，而不只是装饰一个固定支架。", [18]),
    ("因此，OVAL 成为一个尤其合适的检验对象，因为它丰度高、与矿化相关，而且在早期成壳过程中已被提示具有 Ca²⁺ 响应构象行为。", [4, 11, 18, 29]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("因此，我们把比较锚定在保守的破壳齿界面上，并检验共享基质蛋白上的糖链状态差异是否会在矿化起始时产生不同的 Ca²⁺ 可接近表面。", [4, 16, 18]),
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ([ ("这一设计在一个保守的出壳框架内交叉了发育与生态差异，而不是把比较简化为单纯系统发育排序。", False, False)], []),
            ([ ("我们结合显微 CT 形态测量、比较蛋白组学与完整糖肽质谱、Re-Glyco 建模及静电分析和有限元模拟，把壳体结构一路追踪到分子状态再到力学结局。", False, False)], []),
            ([ ("沿着这条证据链，OVAL 给出了最一致的跨尺度信号：其糖链状态与乳突密度、Ca²⁺ 相关表面可及性和局部出壳抗性一致。", False, False)], [18]),
])
