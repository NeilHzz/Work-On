...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳首先面对的是一个很简单的力学问题：胚胎必须把力集中到一个局部破壳位点。", [16, 38, 86]),
    ("在鸟类中，这个位点由短暂存在的破壳齿定义，它从壳内侧顶压蛋壳并触发出壳。", [16, 38, 86]),
    ("如果破壳齿的功能在不同鸟类之间总体保守，那么真正有意义的出壳差异就更可能来自蛋壳，而不是来自破壳工具本身。", [16, 38, 86]),
    ("鸟类蛋壳会随孵化环境和生活史策略变化，而不是在鸟类中呈现一种统一方案。", [15, 39, 87]),
    ...
    ("乳突层是最适合开始分析的层级，因为它是最先影响力学的蛋壳层，其乳突结节标记了方解石在蛋壳基质蛋白调控下开始生长的位置。", [1, 4, 28]),
    ("因此，蛋壳让我们能够在同一个结构里同时解读基质化学、微结构和与出壳相关的力学。", [1, 16, 38]),
...
p_intro2 = spara([
    ...
    ("但这些蛋白并不只通过丰度发挥作用。正如Zeng等所指出，同一种蛋壳基质蛋白可处于不同的N-糖基化状态，这意味着糖基化程度差异可在不整体替换蛋白的情况下改变生物学功能。", [18]),
    ("但鸟类蛋壳研究很少在跨物种框架中解析共享基质蛋白具体携带哪些N-糖链形式。", [18]),
    ("这一缺失限制了比较解释：以往蛋壳比较研究大多根据蛋白身份或丰度推测机制，却无法直接观察这些共享蛋白携带的具体糖链形式。", [18]),
    ("因此，真正缺失的一层并不是基质蛋白是否重要，而是共享基质蛋白上的糖基化能否解释为什么相似的蛋白工具箱会产生不同的蛋壳结构。", [2, 4, 18]),
])

p_intro_sig_cn = spara([
    ("这一缺口之所以重要，是因为糖基化可影响多种蛋白性质，包括稳定性、分子间识别、表面暴露和构象调控；其中，折叠状态在这里尤其重要。", [49, 50]),
    ...
    ("如果这种结构差异具有生物学意义，那么在出壳相关的力学终点上也应当仍可见，即破壳齿样加载下乳突界面的局部抗性。", [16, 17, 38]),
])
...
p_intro4 = smixed([
    ([ ("本文比较了", False, False),
            ...
            ("Columba livia", False, True),
                ("，分别作为陆生早成型、强水域关联早成型和陆生晚成型模型，从而在共同的出壳框架内跨越发育与生态两个交叉对比维度。", False, False)], [22, 23, 24]),
            ([ ("这一设计避免了比较坍缩为简单的系统发育配对，或单一的“早成对晚成”对照。", False, False)], [22, 24, 25]),
...
p_disc_mam1_cn = smixed([
    ([('结果界定出一条一致的比较顺序。由于三种鸟的破壳齿功能保持保守，最有信息量的差异只能在蛋壳中寻找。第一层与力学相关的差异出现在乳突层，而共享基质工具箱随后把比较聚焦到糖基化，而不是整体蛋白替换。', False, False),
            ('在这条链中，OVAL糖链状态为分子变化、表面可及性和局部出壳抗性之间提供了最清楚的比较对应。', False, False)], [1, 16, 18]),
])

p_disc_regulator_cn = spara([
    ("这一比较设计之所以重要，是因为蛋首先服务于后代繁殖，而与蛋最直接相关的并不是某个二值标签，而是巢穴环境和雏鸟状态这两条连续轴线。", [15, 39]),
    ("巢穴环境与陆生到强水域依赖的梯度高度相关，雏鸟状态则从更早成到更晚成连续分布，因此两者都不适合被压缩为简单的二分法。", [23, 24]),
    ("前人的比较讨论往往把栖息环境关联或发育方式当作蛋壳生物学的类别标签来使用；本研究是在这些工作的基础上进一步把两者都作为连续轴线处理，并有意选取该共享空间中彼此分离的区域。", [15, 24]),
    ("鸭正因此避免了这一比较被压缩成简单的“早成对晚成”对照：它保留了广义上的早成发育条件，但OVAL糖链状态和可及性谱型转向中间状态，τ_max结果也与鸽而非鸡收敛。", []),
    ("换句话说，这组三物种的价值不在于构成整齐类别，而在于它们在生态—发育连续空间中被有意拉开，从而尽量减少中间组合对比较解释的干扰。", [22, 24]),
], before=0, after=120)
...
p_disc_axis_cn = spara([
    ...
    ("这种强调与前人把乳突层视为晶体成核与基质调控交汇处的认识是一致的，但本研究进一步把这一层直接连到一个可跨物种比较的糖链状态读数上，而不只是停留在壳体质量描述层面。", [1, 28]),
...
p_disc_other_cn = spara([
    ...
    ("这些结果并不意味着一个蛋白就能解释整个蛋壳；相反，在当前数据中，OVAL提供了一条最清晰的比较路径，使我们能够合理推想基质修饰如何进一步传递到结构与功能层面。", []),
], before=0, after=120)
...
p_disc_oval_cn = spara([
    ...
    ("尽管这一结果并不建立矿化的直接因果机制，但它支持一种有边界的推论：同一基质蛋白上的不同糖链状态可能改变呈现给成矿环境的化学表面，并因此合理参与这里观察到的结构分化。", [49]),
], before=0, after=120)

p_disc_mech_cn = spara([
    ("力学分析把同一模式延伸到个体层面的功能背景。", [16, 17, 38]),
    ("比起原始破裂力，τ_max更能捕捉乳突层级的力学差异。", [34, 38]),
    ("这一点也构成了与多数前人蛋壳力学研究的区别：以往工作更常强调破裂力、壳体强度或整壳质量指标，而这里真正要问的是，一个局部乳突界面指标是否会恢复与分子和结构比较相同的对比格局。", [34, 35]),
    ("绝对失效载荷仍受壳厚和整壳几何影响，而τ_max更直接地读出出壳相关加载下乳突界面的局部抗性。尽管鸭和鸽的总体乳突几何不同，二者在τ_max上却收敛，这说明一旦缺少鸡那种高密度状态，仅靠更下游的形态变化并不能恢复同等级别的乳突界面抗性。", [17, 38]),
    ("因此，相比整壳受力，τ_max更适合作为与前述分子和形态结果并列解读的功能指标。", [34, 38]),
])
...
p_disc_discriminate_cn = spara([
    ...
    ("壳厚、体型和广义繁殖生态都可能提供背景差异，谱系历史无疑也重要。", [14, 24]),
    ("但基于厚度的解释无法说明τ_max差异，而弥散的谱系分化解释也无法说明为什么同一对比会在糖链类别、静电可及性和乳突层组织中反复出现。", [16, 38]),
