...
OUT = str(Path(__file__).with_name("manuscript0602_cn.docx"))
...
para(
    "鸟类依靠功能保守的破壳齿出壳，因此尚未解释的局部破壳力学差异更可能位于蛋壳，而不是破壳工具本身。"
    "我们比较鸡、鸭和鸽，检验分化是否首先出现在乳突层，以及共享基质蛋白上的糖链状态是否追踪这种分化。"
    "我们整合显微CT形态测量、蛋壳基质蛋白组学、完整糖肽质谱、Re-Glyco建模、静电分析和有限元模拟。"
    "乳突层组织发生分化，而基质蛋白工具箱整体仍然广泛共享。"
    "在这些共享蛋白中，卵清蛋白OVAL呈现有序的糖链状态变化：鸡以高甘露糖型为主，鸭以中性复合/杂合型为主，鸽以唾液酸化复合/杂合型为主。"
    "这一变化与逐步降低的Ca²⁺相关表面可及性一致，也与模拟局部出壳抗性中鸡相对于鸭和鸽的同一对比一致。"
    "综上，这组结果支持OVAL糖链状态是连接鸡式蛋壳状态、乳突层组织和局部出壳力学的一个近端分子层。",
    bold=False, size=10, before=0, after=80, align=WD_ALIGN_PARAGRAPH.JUSTIFY
)

para(
    "导语：紧凑的OVAL糖链标记了高抗性的鸡式蛋壳状态。",
...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳面对的是一个局部化的力学问题：胚胎必须把力准确传递到一个很小的破壳位点。在鸟类中，这个位点由暂时存在的破壳齿定义，它从壳内侧顶压蛋壳。", [16, 86]),
    ("类似的辅助出壳结构也见于其他卵生羊膜动物。", [82, 83, 84, 85]),
    ("正因为破壳齿的功能总体保守，真正有生物学意义的出壳差异更可能位于蛋壳，而不是破壳工具本身。", [16]),
    ("鸟类蛋壳会随巢穴环境、气体交换、微生物暴露压力以及发育方式而变化。", [15, 26, 39]),
    ("因此，比较分析并不会恢复出一种单一的统一结构方案。", [40, 41]),
    ("在这一背景下，乳突层尤为关键，因为它是蛋壳中最先对力学产生直接影响的层级，也是蛋壳基质调控下方解石开始生长的位置。", [1, 4, 20, 28]),
    ("后续壳层会继承这一早期成矿背景。", [30, 34]),
    ("因此，乳突层组织正是局部分子差异最可能被放大为成熟蛋壳行为差异的位置。", [1, 57]),
])

p_s1b_cn = spara([
    ("因此，真正的机制问题就变成：当破壳界面被固定后，究竟是哪一类调控乳突层组织的分子因素，能够解释不同物种之间恢复出的不同蛋壳状态？", [1, 4, 16]),
    ("更早关于蛋壳基质和乳突层的研究，也已经把这一层级界定为最可能的控制点。", [2, 28]),
])

# §2 — 已知工作与局限
p_intro2 = spara([
    ("蛋壳基质蛋白调控乳突层成矿、晶体生长和成熟蛋壳结构。", [1, 2, 4]),
    ("其中OC17、OC116、TRFE和OVAL是研究最充分的代表。", [10, 19, 21, 29]),
    ("近期综述一再恢复出一组彼此重叠的反复出现蛋白，包括OC17、OC116、ovotransferrin相关组分、OVAL、ovomucoid和ovocalyxin家族蛋白。", [1, 2, 4]),
    ("它们的反复出现，使其成为跨研究讨论蛋壳组织的稳定锚点。", [19, 21, 29]),
    ("因此，这些蛋白为检验共享成矿工具箱如何在不同壳体背景中被重复使用提供了天然比较锚点。尚待解决的关键问题，不是蛋白是否存在，而是这些共享蛋白如何被部署。", [1, 2, 4]),
    ("目前已知的蛋壳基质蛋白后修饰主要集中在磷酸化和糖基化两类，这两类修饰的位点都已经得到较广泛研究。", [17, 18, 21]),
    ("但两者的一个关键差别在于化学多样性：磷酸化侧链基团彼此相对相似，而糖基化侧链在组成、大小和电荷上差异很大，因此对蛋白生物学功能的影响也并不相同。", [49, 50, 80]),
    ("因此，针对糖基化的研究不能只停留在修饰位点层面，位点上所携带的具体糖链类别本身就是核心变量。", [49, 50]),
    ("早期糖蛋白组学研究显示，同样的蛋壳基质蛋白可以处于不同的N-糖基化状态。", [18]),
    ("以鸡为主的生化研究也已鉴定出OC116等蛋白中的糖基化Asn，并给出了OVAL相关糖链的碳水化合物组成。", [8, 21]),
    ("围绕蛋清、卵索、卵黄膜和孵化阶段转换的平行研究同样表明，糖基化状态可以在鸟类卵系统内部发生变化。", [7, 47, 48]),
    ("这些研究至关重要，但它们通常按单一物种、单一壳层区室或单个位点目录来组织。因此，它们证明了蛋壳糖基化确实存在且具有化学多样性，却尚未显示共享基质蛋白上的匹配糖链状态如何与跨物种蛋壳结构对齐。", [8, 18, 21]),
    ("许多反复出现的蛋壳蛋白和糖基化位点已经被认识。", [1, 2, 29, 66]),
    ("真正尚未解决的是，当乳突层组织和破壳力学在物种间发生分化时，这些共享蛋白如何在糖链状态层面被重新部署。", [18]),
...
p_intro_sig_cn = spara([
    ...
    ("在其他系统中，糖链也可以作为动态遮蔽层，而不只是被动附着的体积。", [42, 43, 44, 63]),
    ("Zeng及其同事进一步表明，同一种蛋壳基质蛋白在角质层和矿化层之间可以处于不同的N-糖基化状态。因此，糖链状态可能重新分配蛋白在不同蛋壳区室中的生物学角色，而不是仅仅装饰一个固定的蛋白支架。", [18]),
    ("既往与矿化相关的工作提示，OVAL在早期成壳过程中可能进入Ca²⁺响应的构象状态。", [4, 11, 29]),
    ("其他糖基化系统中的工作也进一步表明，糖链差异可以重塑折叠蛋白的表面。", [42, 43, 61]),
    ("相关研究还显示，糖链会影响可接近界面及其遮蔽样表面行为。", [63, 81]),
    ("因此，我们进一步检验跨物种糖链差异是否会重塑折叠OVAL的表面，并改变矿化起始时呈现给Ca²⁺的可接近界面。", [4, 18]),
...
p_s0b_cn = mixed([
    ...
    ("G. gallus", False, True),
    ("最低（0.40 ± 0.01）。从定量上看，鸡表现出最致密的早期成矿格局。鸽把最大比例的壳体体积分配给由单个乳突knobs生长出的晶体单元，而鸭在晶体单元占比上居中、在密度上则更接近鸽。这两个指标并没有压缩成一条单调轴线，但它们都表明，在考虑更晚出现的壳体特征之前，乳突层组织已经发生分化。这一层是最早支配蛋壳力学的结构层级，并受基质蛋白调控。因此，我们进一步提出一个更窄的问题：这种对比究竟反映了工具箱的整体替换，还是一个大体共享系统内部的差异性使用？", False, False),
])
...
add_main_figure_legend(
    ...
    [
        ...
        (" 和 ", False, False),
        ("Columba livia", False, True),
        ("。(B) 三物种侧面头部视图和背侧喙部视图，显示带破壳齿的喙尖。(C) 乳突层代表性显微CT截面和三维内表面重建；鸡表现为更平滑的圆钝乳突，鸭表现出更多棱脊和角状结构，鸽则以离散三角锥状乳突为主。比例尺，100 μm。(D) 乳突密度和晶体单元体积比例的箱线图。散点表示单个测量值；p值来自单因素ANOVA，不同字母表示Tukey HSD分组。", False, False),
    ],
    before=20,
    after=80,
)
...
p_disc_mam1_cn = smixed([
    ([('三物种之间首先分化的是乳突层组织，而蛋壳基质工具箱本身仍然大体共享。', False, False),
            ('在这一共享背景下，OVAL 糖链状态为分子变化、表面可及性和局部出壳抗性之间提供了当前数据中最稳定的对应关系。', False, False)], [1, 16, 18]),
])
...
p_disc_close_cn = para(
    "总之，本研究把三种鸟类蛋壳中的乳突层组织、糖蛋白状态、表面可及性和局部出壳力学连接了起来。鸡定义了这一轴线上的高抗性端点，同时具有致密的乳突组织、紧凑的OVAL糖链、更高的Ca²⁺相关表面暴露，以及乳突界面最高的局部抗性。随着可比糖型归属在其他高丰度蛋壳基质蛋白上逐步可得，同样的策略也可以继续扩展。综合形态测量、糖蛋白组学、结构和力学证据，这组结果支持OVAL糖链状态是一个与鸡式蛋壳状态、乳突层组织和局部出壳力学稳定一致的分子层。",
    bold=False, size=11, before=0, after=120
)
