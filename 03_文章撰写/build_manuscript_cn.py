...
p_s3a_cn = spara([
    ("OVAL随后被作为最强共享候选，用于检验其糖链类别如何改变生物物理可及性。既往体外研究已经表明，OVAL在矿化条件下能够结合Ca²⁺、发生部分解折叠并参与早期矿化相关组装，因此其糖链状态构成了一个合理的控制层。Re-Glyco与APBS分析显示，糖基化会在物种内改变OVAL的性质，而去糖基化后的蛋白骨架在物种间则更为接近。", [6, 7, 11, 12, 18, 42, 43]),
    ("这一重建比较覆盖了三物种中主导检出的糖链类别，因此我们能够在同一结构框架下，而不是在彼此孤立的模型之间，评估物种特异性的糖链几何。", [11, 12, 18, 42, 43]),
    ("一旦去除糖链，大部分跨物种分离便明显收敛，说明大多数跨物种差异主要由糖基化而不是蛋白骨架本身引入。", [11, 12, 42, 43]),
    ("进一步的糖链几何分析表明，鸽的糖链占据最大的构象空间并产生最强的表面遮蔽，鸡的糖链最紧凑且遮蔽最弱，鸭居中。相应地，Ca²⁺相关可及性从鸡到鸭到鸽逐步下降。因此，不同糖型并不只是化学标签，它们确实改变了同一个共享基质蛋白呈现给矿化环境的表面状态。", [11, 12, 42, 43, 44, 45, 46, 49, 50, 51]),
    ("这些表面读数是全文第一次把共享蛋白上的分子差异，翻译成一个可以被空间化描述的表面后果。", [11, 12, 42, 43, 49, 50, 51]),
    ("这些结果并不直接证明糖链决定矿化，但它们确实更直观地说明：同一个共享蛋白的不同糖链形式，可以通过改变表面可及性而形成一种结构上可解释的功能差异。", [11, 12, 42, 43, 49, 50, 51]),
    ("因此，再糖基化结果不只是把三物种区分开来，而是进一步指出，真正负责这一区分的物理特征是共享酸性界面周围表面遮蔽程度的不同。", [11, 12, 42, 43, 44, 45, 46, 49, 50, 51]),
])
