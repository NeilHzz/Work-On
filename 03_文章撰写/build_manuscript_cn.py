...
para(
    "以往对蛋壳的比较描述，主要停留在蛋白组成和糖基化位点层面，因此共享基质蛋白在糖链状态层面如何被重新部署、并进一步关联蛋壳结构与出壳力学差异，仍缺乏直接解释。"
    "在这里，我们把鸡、鸭和鸽作为被有意拉开的生态—发育状态，并放在一个保守的破壳齿界面下比较。"
    "我们整合显微CT形态测量、蛋壳基质蛋白组学、完整糖肽质谱、Re-Glyco结构建模、静电分析和有限元模拟。"
    "最先恢复出的强跨物种分离出现在乳突层组织，而不是整体仍大体共享的基质蛋白工具箱。"
    "在这一共享背景下，卵清蛋白 OVAL 呈现有序的糖链状态变化：鸡以高甘露糖型为主，鸭以中性复合/杂合型为主，鸽以唾液酸化复合/杂合型为主。"
    "这些糖链状态对应着逐步增强的 OVAL 钙相关表面遮蔽，并在局部出壳抗性中再现出鸡高、鸭/鸽低的同一对比。"
    "综合形态测量、糖蛋白组学、结构和力学证据，OVAL 糖链状态成为连接鸡式蛋壳状态、乳突层组织与内向外破坏行为的最可化学解释的分子层。",
...
# §1 — 背景价值
p_s1a_cn = spara([
    ("鸟类出壳首先是一个局部失效问题：力并不是施加在整个壳面上，而是必须穿过一个很小、由破壳器定义的位点。在鸟类中，这个破坏器就是暂时存在的破壳齿，它从壳内侧顶压蛋壳。", [16, 86]),
    ("类似的辅助出壳结构也反复出现在其他卵生羊膜动物中。", [82, 83, 84, 85]),
    ("正因为这一破壳装置的功能总体保守，真正有生物学意义的出壳差异更可能位于蛋壳，而不是工具本身。", [16]),
    ("然而，鸟类蛋壳又会随巢穴环境、气体交换需求、微生物暴露和发育方式而变化。", [15, 26, 39]),
    ("因此，比较研究并不会恢复出一种单一的统一壳体方案。", [40, 41]),
    ("在这样的背景下，乳突层尤为关键，因为它既是蛋壳中最先对力学产生直接影响的层级，也是蛋壳基质调控下方解石开始生长的位置。", [1, 4, 20, 28]),
    ("后续壳层会继承这一最初的成矿背景。", [30, 34]),
    ("因此，乳突层组织就是局部分子差异最早可能被放大为成熟蛋壳行为差异的层级。", [1, 57]),
])
...
# §2 — 已知工作与局限
p_intro2 = spara([
    ("蛋壳基质蛋白调控乳突层成矿、晶体生长和成熟壳体结构。", [1, 2, 4]),
    ("其中 OC17、OC116、TRFE 和 OVAL 仍是研究最充分的代表。", [10, 19, 21, 29]),
    ("近期综述一再恢复出一组彼此重叠的反复出现蛋白，包括 OC17、OC116、ovotransferrin 相关组分、OVAL、ovomucoid 和 ovocalyxin 家族蛋白。", [1, 2, 4]),
    ("它们的反复出现，使其成为跨研究讨论蛋壳组织的稳定比较锚点。", [19, 21, 29]),
    ("因此，真正尚待解决的问题，不是共享成矿工具箱是否存在，而是它如何被差异化部署。", [1, 2, 4]),
    ("目前已知的蛋壳基质蛋白后修饰主要集中在磷酸化和糖基化两类，这两类修饰的位点都已经得到较广泛研究。", [17, 18, 21]),
    ("关键的不对称在于化学范围：磷酸化侧链彼此相对相似，而糖链在组成、大小和电荷上变化更大，因此可能以不同方式重塑蛋白行为。", [49, 50, 80]),
    ("因此，针对糖基化的研究不能停留在位点占据层面，位点上附着的糖链类别本身就是机制变量。", [49, 50]),
    ("早期糖蛋白组学研究显示，同样的蛋壳基质蛋白可以处于不同的 N-糖基化状态。", [18]),
    ("以鸡为主的生化研究也已鉴定出 OC116 等蛋白中的糖基化 Asn，并给出了 OVAL 相关糖链的碳水化合物组成。", [8, 21]),
    ("围绕蛋清、卵索、卵黄膜和孵化阶段转换的平行研究同样表明，糖基化状态可以在鸟类卵系统内部发生变化。", [7, 47, 48]),
    ("这些研究至关重要，但它们通常按单一物种、单一壳层区室或单个位点目录来组织。它们证明了蛋壳糖基化确实存在且具有化学多样性，却尚未解析共享基质蛋白上的匹配糖链状态如何与跨物种壳体结构对齐。", [8, 18, 21]),
    ("许多反复出现的蛋壳蛋白和糖基化位点已经被认识。", [1, 2, 29, 66]),
    ("真正尚未解决的是，当乳突层组织和破壳力学在物种间发生分化时，这些共享蛋白如何在糖链状态层面被重新部署。", [18]),
    ("鸟类蛋壳研究很少解析共享基质蛋白在跨物种背景下究竟携带哪些具体 N-糖链形式。开放的问题因此是，这一糖链层是否解释了为什么相似的蛋白工具箱会产生不同的蛋壳状态。", [2, 4, 18]),
])
...
p_intro4 = smixed([
    ([ ("在这里，我们比较了", False, False),
            ...
            ([ ("这一设计在共同出壳框架内同时抽样发育与生态的交叉对比，而不是把比较压缩成单纯的系统发育或早成对晚成二分。", False, False)], []),
            ([ ("我们整合显微CT形态测量来界定乳突层组织，结合比较蛋壳基质蛋白组学和完整糖肽质谱来解析共享基质蛋白及其糖链状态，再用 Re-Glyco 结构建模和静电分析推断表面后果，并用有限元模拟检验同一对比是否仍能在局部出壳抗性中被检测到。", False, False)], []),
            ([ ("每一层证据都约束下一层，使分子解释始终被壳体结构和力学结果锁定，而不是脱离材料背景。", False, False)], []),
            ([ ("沿着这条顺序路径，OVAL 提供了最清楚的跨尺度联系：其糖链状态与乳突密度、Ca²⁺ 相关表面可及性和局部出壳抗性一致。", False, False)], [18]),
])
...
p_disc_mam1_cn = smixed([
    ([('在当前比较中，乳突层组织呈现出最清晰的早期结构对比，而蛋壳基质工具箱本身仍然大体共享。', False, False),
