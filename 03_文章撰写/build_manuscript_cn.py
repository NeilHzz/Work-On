...
para(
    ...
    "综上，OVAL糖链状态是连接鸡式蛋壳状态、乳突层组织和局部出壳力学的最清晰分子层。",
...
para(
    "所有数值均表示为mean ± s.d.，所有统计检验均为双侧，p < 0.05视为显著。参数性物种间比较前，先用Shapiro-Wilk检验正态性，并用Levene检验方差齐性。乳突层形态参数在满足前提后，采用单因素ANOVA结合Duncan多重范围检验（DMRT；α = 0.05）比较（每物种n = 9个子区域）。相同的前提检验同样支持对有限元结果（F_max和τ_max）采用单因素ANOVA结合Duncan多重范围检验。相比之下，糖链系综几何描述符（Rg、端到端距离、最小糖链-蛋白接触距离）和热点衍生的系综指标在物种间不满足正态性和/或方差齐性，因此这些变量的两两物种比较采用双侧Mann-Whitney U检验。"
    "在 ",
    bold=False, size=11, before=0, after=120
)
