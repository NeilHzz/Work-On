...
p_intro4 = smixed([
    ...
    ([ ("也就是说，这套流程的设计本身就要求每一层证据去约束下一层解释，而不是让分子层面的判断脱离具体壳体结构独立悬浮。", False, False)], [3, 22, 24, 25, 42]),
...
p_s3a_cn = spara([
    ...
    ("进一步的糖链几何分析表明，鸽的糖链占据最大的构象空间并产生最强的表面遮蔽，鸡的糖链最紧凑且遮蔽最弱，鸭居中。相应地，Ca²⁺相关可及性从鸡到鸭到鸽逐步下降。也就是说，不同糖型并不只是化学标签，它们确实改变了同一个共享基质蛋白呈现给矿化环境的表面状态。", [11, 12, 42, 43, 44, 45, 46, 49, 50, 51]),
...
p_s4a_cn = mixed([
    ("结果开头所界定的共同破壳齿界面，随后被进一步转化为有限元检验中的显式加载设计。图5A给出了与出壳相关的加载背景，而图5B-D则把三种鸟类喙部俯视图与对应的micro-CT有限元建模并列展示。由于网格保留了来自micro-CT的物种特异性壳体几何，这一力学检验始终锚定在前文已经识别出的乳突层背景之上，而不是退化成一个理想化均质壳壁模型。这里我们真正关心的不是广义整壳强度，而是前文恢复出的分子和结构对比，是否仍保留在乳突接触界面的局部抗性中。也就是说，这里的有限元分析承担的是一个跨尺度检验：前文由基质蛋白糖链状态、OVAL表面可及性和乳突组织所推得的对比，是否仍能在真实出壳力学界面上被检测出来。"
