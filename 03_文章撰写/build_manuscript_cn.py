...
p_intro2 = spara([
    ...
    ("这一点在最近的鸟类古蛋壳蛋白研究中也表现得非常清楚。Codlin 等人特意优先整理了 c-type lectins XCA1/XCA2、OC116、albumin、BPIFB4、clusterin、lactadherin、OVAL、ovocalyxin-32、ovomucoid 和 ovotransferrin 等蛋壳蛋白，因为这些分子在降解样品中仍具有足够的丰度、重复回收率和可比较性，能够支撑古蛋壳材料中的分类与比较分析。对我们而言，这份选择同样有意义，因为它指出了哪些蛋白最适合作为稳定比较锚点，而不是容易陷入个体噪音的偶发信号。", [29, 66]),
...
p_sprot_focus_cn = spara([
    ...
    ("放在这一共享基质框架下，引言中提到的反复出现蛋白并不是等量地都被当前数据保留下来：只有 OVAL、OC116 和 TRFE 作为三物种共享糖基化锚点持续存在，OC17 只保留在鸡中，而 ovomucoid、lactadherin、clusterin 以及更广义的 Codlin 优先蛋白集合都没有进入最终的共享结构比较子集。", []),
])
...
p_s3c_cn = spara([
    ("一旦这种几何排序被建立，图4H-K要回答的就不再是四个彼此无关的问题，而是同一片共享酸性界面在连续过滤后还剩下多少可用于矿化。图4H先给出整体界面遮蔽程度；图4I进一步问在这种遮蔽之后还有多少热点表面积保持溶剂暴露；图4J再问候选酸性残基中还有多大比例仍能保留为热点；图4K最后只保留其中同时满足静电有利且空间可及、能够被Ca²⁺接近的净热点。", []),
    ("按这个顺序阅读，图4H-K构成的是一条单向推进的遮蔽级联。界面遮蔽从鸡到鸭到鸽逐步增强，而每一步下游筛选都保留同样的排序，说明更延伸且更贴面的糖链会把同一片共享酸性 OVAL 表面逐步从“大面积可用界面”压缩成更小、更难被矿化环境利用的化学活性区。", []),
])

p_s3d_cn = spara([
    ("图4L和图4M则把同一差异进一步压缩到整体界面层面，分别把热点数量和热点残基表面积拆分为暴露部分与被遮蔽部分。鸡在这两幅图中都保留了最大的暴露份额，鸽把最大的份额转入遮蔽区室，而鸭仍位于两者之间。", []),
    ("把这一结果压缩到整体界面层面后，鸡也就对应于最强的 Ca²⁺ 吸附潜力，并且最符合更早进入 Ca²⁺ 响应式构象打开、从而更早启动矿化的状态；鸽则对应最弱、最晚的另一端，鸭仍处于中间。这一排序与前文表型链条是对上的：鸡同时具有最高的乳突密度和最高的局部出壳抗性，而鸭和鸽则从不同结构背景收敛到较低抗性侧。因此，图4A-M构成的是一条连续的结构论证，而不是若干并列子图：它从糖链依赖的分离出发，经过糖链几何和界面遮蔽，最后收束到共享基质蛋白上的 Ca²⁺ 相关表面可及性。", []),
])
