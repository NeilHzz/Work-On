...
OUT = str(Path(__file__).with_name("manuscript260518v3_cn.docx"))
...
p_intro2 = spara([
    ...
    ("其中，OC17常被作为与方解石相互作用的经典蛋壳蛋白，OC116是与壳体组织密切相关的主要磷酸化且糖基化基质成分，ovotransferrin 相关蛋白处在抗菌保护与成矿相关基质化学的交界处，而 OVAL 则是在多项成壳研究中持续被保留的高丰度基质相关蛋白。", [4, 10, 19, 21, 29]),
    ("从功能位置上看，这些蛋白并不是彼此无关的候选，而是落在同一条成壳序列的不同层面：OC17 和 OC116 更直接对应乳突层及晶体组织，TRFE 和 OVAL 则把这种基质逻辑延伸到早期成壳阶段可溶、表面活跃以及囊泡相关的基质背景。也就是说，这里的比较并不是从零散分子开始，而是从一组已经被放在方解石起始成矿框架中的共享基质参与者开始。", [4, 10, 19, 21, 29]),
    ("也正因为这些蛋白在鸟类蛋壳研究中持续出现，它们构成了检验“共享成矿工具箱如何在不同壳体背景中被重复使用”的天然比较锚点。然而，这些蛋白并不只通过丰度发挥作用。", [1, 2, 4]),
...
p_s3a_cn = spara([
    ...
    ("图4A中的Ca²⁺热点，指的是表面Asp/Glu酸性位点中电势足够负、因而有利于局部钙离子接近的位置；图4B显示这些带羧酸基的表面还有多少保持物理暴露；图4C则将同一变化可视化为整表面电势图。三物种中，糖基化都会使这些读数偏离配对的去糖基化参考，而一旦去除糖链，三物种蛋白骨架之间的大部分分离就会明显收敛。这说明大部分跨物种差异是由糖基化引入，而不是由蛋白支架本身造成（图S7）。", []),
])

p_s3b_cn = spara([
    ("接下来的问题是，这种糖链状态推进在几何上究竟如何落实到 OVAL 表面。鸽首先通过更大的整体糖链包络与另外两种鸟分开，这一点体现在图4D的更高回转半径和图4E更长的端到端距离上。", []),
    ("但这种扩展并没有把糖链整体抬离蛋白表面。相反，在图4F中，鸽的糖链-蛋白距离并未相应增大，而在图4G中还表现出更小的最小糖链-骨架距离，说明这些更长的糖链同时也更容易回折并贴近 OVAL 表面。鸡对应的是另一端：图4D和图4E中的糖链最紧凑，图4F和图4G中的表面接触也最弱；鸭则再次位于两者之间。这样，图4D-G就不再只是逐个解释几何指标，而是把跨物种糖型差异转化为一条遮蔽几何链条：从鸡的紧凑且弱接触糖链，到鸽的延伸但贴面的糖链，再到居中的鸭状态。", []),
])
p_s3c_cn = spara([
    ("图4H-K量化了糖链层遮蔽共享酸性OVAL界面的程度。图4H量化相关酸性界面被糖链遮蔽的程度；图4I给出热点残基的溶剂可及表面积；图4J记录候选酸性残基中仍保留为热点的比例；图4K统计在电势上仍有利于Ca²⁺接近、且在空间上也保持可及的净热点数量。", []),
    ("在这一组指标中，表面遮蔽从鸡到鸭到鸽逐步增强，而暴露热点面积、热点比例和净可及热点数量则沿相反方向逐步下降。三物种的差异因此并不是简单地带有不同糖链，而是这些糖链在不同程度上覆盖了同一片原本能够与局部Ca²⁺发生接触的共享酸性界面（图4H-K）。", []),
])

p_s3d_cn = spara([
    ("图4L和图4M把同一可及性差异进一步拆分为暴露和遮蔽两部分，分别对应热点数量和热点残基表面积。", []),
    ("在这一整体层面，鸡保留了最大的暴露份额，鸽把最大的份额转入了被遮蔽部分，鸭仍然位于两者之间。三种主导OVAL糖型由此形成同一个共享的物理对比：鸡的高甘露糖型使酸性界面保持最开放，鸭的中性复合/杂合型使其部分覆盖，而鸽的唾液酸化复合/杂合型则造成最强的整体遮蔽。综合来看，再糖基化和电势分析把这一跨物种对比解析为从糖链组成、糖链几何、界面遮蔽到Ca²⁺相关表面可及性的递进链条（图4L-M）。", []),
])
...
p_disc_selection_cn = para(
    "鸭和鸽之所以仍然关键，在于它们界定了鸡状态的边界。鸭说明更大的壳厚可以抬高原始受力，却不足以重建相同的乳突界面抗性；鸽则说明更薄的壳体也可以在较低τ_max上与鸭收敛，但达到这一结果所依赖的形态背景并不相同。把这两个对照放在一起，鸡就成为把糖链依赖的基质行为与蛋壳性能连接起来的最清晰参考状态。在这一框架下，OVAL糖链状态是使鸡式高抗性状态获得机制解释的最明确分子层。",
    bold=False, size=11, before=0, after=120
)
