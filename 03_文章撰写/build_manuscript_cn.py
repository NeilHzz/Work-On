...
p_s1b_cn = spara([
    ("基于这一逻辑，我们把乳突层组织作为核心比较表型，并选择鸡、鸭和鸽去覆盖两个被刻意拉开的维度：早成 versus 晚成，以及陆生 versus 水域关联生境。", [4, 13, 15, 16, 23, 26, 39, 41]),
    ("这一定义比较空间的目的不是直接把生态当成解释，而是在共享出壳界面不变的前提下，观察蛋壳生物学如何变化。", [4, 13, 15, 23, 39, 41]),
])
...
p_intro_sig_cn = spara([
    ("把问题锚定到出壳界面之后，这条逻辑链会变得非常清楚。", [16, 17, 38]),
    ("如果破壳齿的功能是保守的，那么真正需要解释的差异就从破壳工具本身转移到了蛋壳。", [16, 17, 38, 82]),
    ("如果乳突层是蛋壳力学的结构起点，那么下一层机制解释就必须回到调控乳突形成的基质蛋白系统。", [1, 2, 4, 28, 38]),
    ("而如果糖基化可以改变同一种基质蛋白的生物学功能，那么糖链状态就成为连接基质组成与蛋壳性能的一个化学特异性层。", [18, 20, 21, 49, 50, 52]),
])

# §3 — 核心缺口
p_intro_gap_cn = spara([
    ("因此，我们真正要问的是：不同发育模式和繁殖背景下的鸟类，是否在共享蛋壳基质蛋白上采用了不同的糖基化形式，以及这些形式是否具有结构和力学后果。", [55, 58, 59, 60, 65]),
    ("更具体地说，当比较被锚定到保守的破壳齿界面之后，糖链状态差异是否与乳突层形态、Ca²⁺相关可及性以及局部出壳抗性保持一致。", [55, 58, 59, 60, 65]),
])
...
p_intro4 = smixed([
    ([ ("为检验这一问题，本文选择三种喙形差异明显、但仍通过同一种破壳齿介导局部接触事件完成出壳的鸟类作为比较对象：", False, False),
            ...
            ("，分别代表陆生早成型、强水域关联早成型和陆生晚成型模型。", False, False)], [3, 22, 24, 25, 82, 83]),
        ([ ("以这一比较框架为基础，本文整合显微CT形态测量、蛋壳基质蛋白组学、完整糖肽质谱、Re-Glyco结构系综建模、APBS静电分析和有限元仿真。", False, False)], [3, 22, 24, 25]),
        ([ ("这一逐层推进的设计，使我们能够从保守的出壳功能走到蛋壳结构，再从乳突层走到基质蛋白、糖链形式、蛋白表面状态，最终走到局部出壳性能。", False, False)], [4, 11, 12, 16, 42]),
