...
p_intro2 = spara([
    ...
    (" Recent eggshell-matrix studies repeatedly recover a partly overlapping set of proteins, including OC17, OC116, ovotransferrin-related components, OVAL, ovomucoid, and ovocalyxin-family members. Their recurrence makes them a practical starting point for comparing shell organization across studies.", [1, 2, 4, 19, 21, 29]),
    (" That recurrence also suggests that avian eggshells are built from a reused matrix repertoire rather than from unrelated protein inventories. The unresolved question is how that shared repertoire is redeployed in different shell contexts, and whether the key difference lies in protein amount or in protein state.", [1, 2, 4]),
    ...
    (" Put differently, the field already knows many of the recurring eggshell proteins and many of the recoverable glycosites. What it lacks is a direct comparison of how those recurrent proteins are redeployed, at the glycan-state level, when mammillary organization and shell-breaking mechanics diverge across species.", [1, 2, 18, 29, 66]),
    (" Avian eggshell studies have rarely resolved which specific N-glycan forms are carried by shared matrix proteins across species. The missing layer is therefore not whether matrix proteins matter, but whether glycosylation on shared matrix proteins can explain why similar protein toolkits yield different eggshell structures.", [2, 4, 18]),
])
...
p_sprot_focus = spara([
    ...
    (" The proteins recovered here were broadly consistent with earlier eggshell-matrix studies, but the overlap was uneven. OVAL, OC116, and TRFE remained the only three-species glycosylated anchors after ortholog filtering; OC17 was restricted to chicken; and proteins such as ovomucoid, lactadherin, and clusterin appeared only in lineage-biased or pairwise-shared sectors. The present dataset therefore extends earlier eggshell-matrix work mainly by resolving glycosylation states on a partly shared repertoire, rather than by greatly enlarging the core matrix-protein list.", []),
])

head("OVAL glycosylation provides the most interpretable cross-species contrast")

p_s2a = mixed([
    ("Intact glycopeptides gave a direct cross-species view of specific glycoforms on avian eggshell matrix proteins. Fig. 2 resolved a conserved three-species core together with pairwise-shared and lineage-restricted sectors. The innermost rings correspond to the three-species shared proteins, the surrounding sectors capture pairwise-shared and lineage-restricted repertoires, and the outer nodes summarize the seven glycan classes. Gray links connect proteins to their observed glycan classes, and darker outer glycan nodes indicate classes linked to larger numbers of proteins. High-Mannose and Complex-Fucosylated glycans formed a broad background across many protein families, whereas more extended sialylated classes concentrated in the peripheral difference nodes. This asymmetry highlighted shared glycoproteins with strongly divergent states and narrowed the candidate space for ortholog and structural analysis.", False, False),
])
...
p_s2c = spara([
    ("Integrated protein and glycan abundance profiles further identified OVAL as the shared protein most closely aligned with the cross-species eggshell differences (Fig. 3B-D). Across the full dataset, protein-glycan coupling was weak in chicken but consistently positive in duck and pigeon, indicating lineage differences in how glycosylation scaled with protein output.", []),
...
p_s3a = spara([
    ...
    (" The first step was to ask whether the cross-species contrast was genuinely glycan dependent. In Fig. 4A, glycosylated ensembles departed from their matched apo references in the number of Ca²⁺-relevant acidic hotspots, and the same displacement reappeared in the physically exposed carboxylate surface measured in Fig. 4B and in the whole-surface electrostatic maps in Fig. 4C. Once glycans were removed, however, much of the separation collapsed and the three backbones clustered more closely to one another. The initial structural difference was therefore not a generic sequence effect, but a glycan-imposed shift in the exposed acidic surface presented at mineralization onset (Fig. 4A-C; Fig. S7).", []),
])

p_s3b = spara([
    (" The next question was how that glycan-state progression was realized geometrically on the OVAL surface. Pigeon first separated by occupying the largest overall glycan envelope, as shown by the higher radius of gyration in Fig. 4D and the longer end-to-end distances in Fig. 4E. That expansion, however, did not lift the glycans away from the protein.", []),
...
p_s3c = spara([
    (" Fig. 4H to K then tracked the same acidic interface through successive filters. Shielding increased from chicken to duck to pigeon in Fig. 4H. The same ordering was mirrored by lower exposed hotspot-residue SASA in Fig. 4I, lower hotspot fraction in Fig. 4J, and fewer net Ca²⁺-accessible hotspots in Fig. 4K.", []),
    (" These were not separate readouts. They were successive views of the same masking process, showing that extended, surface-hugging glycans progressively reduced the chemically usable OVAL interface at mineralization onset.", []),
])
...
p_s3d = spara([
    ...
    (" The same summary also placed chicken at the strongest inferred Ca²⁺-capturing end of the comparison and thus at the state most compatible with the earliest entry into the Ca²⁺-responsive OVAL configuration at mineralization onset. Pigeon defined the opposite limit, and duck again remained intermediate. The same ordering matched the phenotype readout: chicken combined the densest mammillary field and the highest local hatching resistance, whereas duck and pigeon grouped on the lower-resistance side. Together, Fig. 4A-M trace one continuous argument from glycan-dependent separation to glycan geometry, interface masking, and Ca²⁺-relevant accessibility.", []),
])
...
p_s4a = mixed([
    ("Finite-element testing translated the shared egg-tooth interface into an explicit inside-out loading design. Fig. 5A summarizes the loading background of hatching. Fig. 5B-D pair species-specific dorsal beak views with the corresponding micro-CT-derived finite-element setups built from the beak-tip geometry summarized in Fig. 1B. Because the meshes preserved species-specific shell geometry, the analysis remained anchored to the same mammillary context identified morphologically. Impact loading was sampled on circular eggshell fragments (model diameter D = 2.0 mm) at nine parameterized lateral-offset positions on a 3 × 3 grid (0.5 mm spacing), yielding n = 9 independent contact shear-stress time courses per species. Both raw peak contact force (F_max) and peak contact shear stress (τ_max) were recorded to reduce the influence of model size, gross geometry, and especially eggshell thickness. Peak τ_max was used as the direct readout of local hatching resistance at the mammillary contact interface, and species means ± s.d. were calculated across the nine positions (Fig. S8A-F; eggshell thicknesses: ", False, False),
    ("G. gallus", False, True),
    (" 0.29 mm, ", False, False),
    ("A. platyrhynchos", False, True),
...
p_disc_regulator = spara([
    ...
    (" Duck is particularly informative in that design because it retains a broadly precocial developmental condition while shifting toward an intermediate OVAL glycan state and accessibility profile, with a τ_max outcome that converges with pigeon rather than with chicken. The three-species comparison is therefore informative because it samples deliberately separated regions of a continuous ecological-developmental space while keeping the hatching interface itself comparable.", [3, 15, 22, 23]),
])
...
p_disc_selection = para(
    "Duck and pigeon remain essential because they bound the chicken state across both shell structure and ecological-developmental position. Duck combined a more aquatic setting and broadly precocial development with a thicker shell, intermediate OVAL accessibility, and low τ_max, showing that shell thickness alone did not recreate the chicken state. Pigeon approached the same low-τ_max end point from a thinner shell, a more terrestrial context, and a more altricial developmental background. These contrasts make chicken the clearest reference state for linking glycan-dependent matrix behavior to eggshell performance across the sampled ecological-developmental space. Within that frame, OVAL glycan state is the most explicit molecular layer through which the chicken-like high-resistance state becomes mechanistically interpretable."
)

p_disc_biomineral = para(
    "The same analytical sequence should also be testable beyond avian eggshell. Many biomineralized systems depend on organic matrices that regulate ion access, surface exposure, and mineral nucleation through chemically specific interfacial states. The present workflow therefore offers a route from glycoproteomic state to surface presentation and mesoscale function. That logic may also prove useful in regenerative settings, where eggshell-derived materials and eggshell membrane proteins have already been explored for tissue engineering and bone repair, including improved osteoproductivity of injectable grafts. The value of the present study is thus not limited to one avian shell phenotype; it also provides a framework for asking how matrix state organizes mineral behavior across biological and biomedical materials."
)
