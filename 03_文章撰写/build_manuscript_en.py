...
p_s3a = spara([
    ...
    (" Representative rebuilt glycan conformations and species-specific surface maps showed that the dominant glycans occupied different spatial envelopes on the same folded protein scaffold (Fig. 4A and B). In panel A, points 1 and 2 mark the two ends of the glycan end-to-end vector, point 3 marks the glycan centroid, point 4 marks the protein Cα centroid, and the translucent sphere visualizes the radius-of-gyration envelope centered on the glycan centroid. Panel B then translates that geometry into surface exposure by coloring Ca²⁺-relevant regions and blackening the same regions after shielding.", []),
])

p_s3b = spara([
    (" Pigeon first separated by occupying the largest overall glycan envelope, as shown by the higher radius of gyration in Fig. 4C and the longer end-to-end distances in Fig. 4E. If one imagines the vector from point 1 to point 2 in panel A stretching farther around the same centroid at point 3, the same glycan is already sampling a larger spatial reach before it is measured against the protein anchor at point 4. That expansion occurred together with closer local approaches to the backbone in Fig. 4D and a broader glycan-protein distance distribution in Fig. 4F, consistent with extended glycans that sample a larger envelope while still folding back toward the OVAL surface.", []),
    (" Chicken defined the opposite endpoint, with compact glycans and the weakest geometric intrusion into the acidic interface. Duck remained closer to chicken in radius of gyration and overall glycan-protein spacing, but it separated from both species in end-to-end span and minimum backbone approach. Fig. 4C to F thus translate glycan-class progression into a shielding geometry: the smaller the translucent sphere in panel A and the shorter the 1-2 span around point 3 relative to point 4, the more compact the glycan state; the larger the sphere and longer 1-2 span, the more extended and surface-engaging the glycan state.", []),
])

p_s3c = spara([
    (" Fig. 4G to J resolved the same acidic interface at progressively stricter levels. In panel B, the colored patches correspond to Ca²⁺-relevant regions that remain accessible, whereas black marks the same regions after shielding. Fig. 4G therefore measures gross interface shielding, Fig. 4H asks what fraction of candidate acidic residues remained hotspots, Fig. 4I measures the surface area retained by hotspot residues, and Fig. 4J counts the subset of Ca²⁺ hotspots that remained both electrostatically favorable and physically reachable.", []),
    (" Interface shielding increased stepwise from chicken to duck to pigeon, and the same ordering was retained across hotspot surface area, hotspot fraction, and net accessible Ca²⁺ hotspots. Together, these panels show progressive masking of the shared acidic OVAL face during early mineralization; the letter groups on the plots indicate Tukey HSD separation at p < 0.05, with *** denoting p < 0.001.", []),
])

p_s3d = spara([
    (" Matched glycosylated-versus-apo comparisons then showed that glycan addition changed the number of Ca²⁺-relevant hotspot residues and the exposed carboxylate surface most clearly in pigeon (Fig. 4K and L; Fig. S10). Interpreting these comparisons with panel B in mind, glycosylation preserves or hides the same Ca²⁺-relevant patches rather than creating new ones de novo: the colored patches are the sites that remain reachable, whereas the black patches are the same sites after shielding. Duck shifted in the same direction without a resolved structure-level significance call, and chicken could be assessed only descriptively because one glycosylated structure was available. This pattern is more consistent with a glycan-imposed shift in the acidic surface presented at mineralization onset than with a generic sequence effect alone.", []),
