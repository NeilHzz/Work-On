...
OUT = str(Path(__file__).with_name("manuscript_results_sa_polished.docx"))
...
p_sprot_a = mixed([
    ...
    ("C. livia", False, True),
    (" branched separately, indicating that the matrix-protein repertoire itself is sufficient to discriminate the three comparison lineages.", False, False),
])
...
p_sprot_pair = mixed([
    ...
    "binding (MF; both p < 10⁻²⁵), alongside the Wnt signaling pathway and "
     "signal transduction (BP; Fig. 2c). In the present three-species framework, these enrichments are better treated as comparative background than as direct molecular readouts of any single calcium-acquisition mode. The ", False, False),
    ("G. gallus", False, True),
    ("–", False, False),
    ("A. platyrhynchos", False, True),
    (" set—shared by the two precocial Galloanserae but absent in "
     "the altricial pigeon—was enriched for adaptive immune response "
    "and spermatogenesis (BP; Fig. 2b), suggesting that the two precocial species retain a larger immune-defense burden at the eggshell/outer-membrane interface; in pigeon, by contrast, antimicrobial deployment is likely shifted more strongly toward egg-white proteins. The ", False, False),
    ("G. gallus", False, True),
    ("–", False, False),
    ("C. livia", False, True),
    (" set—proteins shared by chicken and pigeon but absent in duck—"
     ...
     "Fig. 2d), suggesting partially shared organizational demands in oviductal secretion and intracellular trafficking. Taken together, these enrichment results provide comparative context, but they do not by themselves identify the specific molecular layer responsible for the structural phenotype.", False, False),
])
cite(p_sprot_pair, [15, 26, 29])

p_sprot_species = mixed([
    ("Species-exclusive proteins further sharpened this contrast and highlighted glycan-processing capacity as a distinctive chicken feature (Fig. 2e). The nine ", False, False),
    ("G. gallus", False, True),
...
p_s2c = spara([
    ("Integrating protein abundance and glycan abundance into the same analytical frame then clarified why OVAL, rather than OC116 or TRFE, best tracked the ordered eggshell phenotype (Fig. 4A-C). At the whole-dataset level, protein-glycan coupling was weak and non-significant in chicken (Spearman rho = 0.15, P = 2.68e-01), but became clearly positive in duck and pigeon (rho = 0.42 and 0.47; both P ≈ 2 × 10⁻⁹), indicating that glycosylation investment scaled more tightly with protein expression in the latter two lineages.", []),
...
head("Re-Glyco ensembles reveal an OVAL accessibility gradient")
