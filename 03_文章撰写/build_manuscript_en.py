...
p_intro2 = spara([
    ...
    (" Recent syntheses repeatedly recover an overlapping set of recurrent proteins, including OC17, OC116, ovotransferrin-related components, OVAL, ovomucoid, and ovocalyxin-family proteins. Their recurrence makes them a stable anchor for shell organization across studies.", [1, 2, 4, 19, 21, 29]),
    ...
    (" Earlier glycoproteomic work showed that the same eggshell-matrix proteins can occupy distinct N-glycosylation states. Chicken-focused biochemical studies had already identified glycosylated Asn in proteins such as OC116 and defined the carbohydrate composition of OVAL-associated glycans. Parallel studies of egg white, chalaza, vitelline membrane, and incubation-stage transitions likewise indicate that glycosylation states can shift within avian egg systems.", [7, 8, 18, 21, 47, 48]),
    ...
    (" Many recurrent eggshell proteins and glycosites are already known. What remains unresolved is how those proteins are redeployed at the glycan-state level when mammillary organization and shell-breaking mechanics diverge across species.", [1, 2, 18, 29, 66]),
...
p_intro_gap = spara([
    ...
    (" OVAL provided a tractable test case. Its Ca²⁺-responsive surface behavior was already biologically relevant in earlier mineralization work, and it remained abundant across species. Its dominant glycan classes could also be followed from glycoproteomics into structural modeling.", [4, 18, 29, 42]),
])
...
p_intro4 = smixed([
    ...
    ([(' We integrated micro-CT morphometry to define mammillary organization. We combined comparative eggshell-matrix proteomics and intact glycopeptide mass spectrometry to resolve shared matrix proteins and their glycan states. Re-Glyco structural modeling and electrostatic analysis were then used to infer protein-surface consequences, and finite-element simulation tested whether the same contrast remained detectable in local hatching resistance.', False, False)], []),
...
p_s0b = mixed([
    ...
    ("G. gallus", False, True),
    (" (0.3975 ± 0.0127). Quantitatively, chicken showed the densest early mineralization pattern. Pigeon devoted the largest share of shell volume to crystal units grown from individual mammillary knobs, and duck remained intermediate in crystal-unit proportion while resembling pigeon in density. The two metrics did not collapse into one monotonic axis, but both showed that mammillary organization had already diverged before later shell traits were considered. Because this layer is the earliest structural level governing eggshell mechanics and is controlled by matrix proteins, we asked a narrower question: did the contrast reflect wholesale toolkit replacement or differential use of a largely shared system?", False, False),
])
cite(p_s0b, [1, 4, 28])
...
p_sprot_focus = spara([
    (" The G. gallus-exclusive set was simultaneously enriched for protein N-linked glycosylation (BP; Fig. S5), shifting the comparison from protein presence to chemical deployment. The retained shared core thus became the relevant molecular background. Glycosylation on shared proteins emerged as the most proximate candidate layer for explaining divergence in mammillary organization and downstream shell behavior.", [18]),
...
p_s3a = spara([
    ...
    (" Representative rebuilt glycan conformations and species-specific surface maps showed that the dominant glycans occupied different spatial envelopes on the same folded protein scaffold (Fig. 4A and B). Matched glycosylated-versus-apo comparisons then showed that glycan addition changed the number of Ca²⁺-relevant hotspot residues and the exposed carboxylate surface. The recovered structural difference therefore reflected a glycan-imposed shift in the acidic surface presented at mineralization onset rather than a generic sequence effect (Fig. 4K and L; Fig. S10).", []),
])
...
p_s4a = mixed([
    ("Finite-element testing translated the shared egg-tooth interface into an explicit inside-out loading design. Fig. 5A pairs species-specific dorsal beak views with micro-CT-derived finite-element setups and summary box plots of peak force and peak shear stress. The setups were built from the beak-tip geometry summarized in Fig. 1B. Because the meshes preserved species-specific shell geometry, the analysis remained anchored to the same mammillary context identified morphologically. Impact loading was sampled on circular eggshell fragments (model diameter D = 2.0 mm) at nine lateral-offset positions on a 3 × 3 grid (0.5 mm spacing). This design yielded n = 9 independent contact shear-stress time courses per species. Both raw peak contact force (F_max) and peak contact shear stress (τ_max) were recorded to reduce the influence of model size, gross geometry, and especially eggshell thickness. Peak τ_max was used as the direct readout of local hatching resistance at the mammillary contact interface. Species means ± s.d. were calculated across the nine positions (Fig. S11A to F; eggshell thicknesses: ", False, False),
    ("G. gallus", False, True),
    (" 0.29 mm, ", False, False),
    ("A. platyrhynchos", False, True),
...
mixed([
    ...
    ("C. livia", False, True),
    (" reached 0.485 ± 0.039 N, and all pairwise differences were significant (Fig. 5A and B). By contrast, τ_max resolved a two-level pattern (p = 6.644 × 10⁻¹⁰). ", False, False),
    ("G. gallus", False, True),
    (" reached 551.6 ± 108.8 MPa and was significantly higher than ", False, False),
    ("A. platyrhynchos", False, True),
    (" at 404.0 ± 39.6 MPa and ", False, False),
    ("C. livia", False, True),
    (" at 393.0 ± 35.2 MPa. The latter two species did not differ significantly from each other (Fig. 5A and C).", False, False),
])

mixed([
    ("The difference between F_max and τ_max showed that duck's higher raw contact force was driven mainly by its greater shell thickness (0.35 mm versus 0.19 mm in pigeon). It did not indicate superior unit-area material resistance. By contrast, ", False, False),
    ("G. gallus", False, True),
...
p_disc_mam1 = smixed([
    ([('Mammillary organization diverged while the eggshell-matrix toolkit remained broadly shared across the three species. ', False, False),
            ('Within that shared background, OVAL glycan state gave the clearest link between molecular variation, surface accessibility, and local hatching resistance in this dataset.', False, False)], [1, 16, 18]),
])
...
p_disc_regulator = spara([
    ...
    (" Duck is particularly informative in that design. It retains a broadly precocial developmental condition while shifting toward an intermediate OVAL glycan state and accessibility profile, with a τ_max outcome that converges with pigeon rather than with chicken. The three-species comparison therefore samples deliberately separated regions of a continuous ecological-developmental space while keeping the hatching interface itself comparable.", [3, 15, 22, 23]),
])

p_disc_axis = spara([
    ("Mammillary-layer mineralization mode remains the central structural level in the interpretation.", [1, 28]),
    (" Once early calcite crystal units are established, later eggshell regions inherit the spacing logic created in that first mineralization window. A dense mammillary field therefore changes matrix retention, mineral continuity, local stress redistribution, and morphology.", [1, 30, 36]),
    (" That emphasis is consistent with earlier eggshell studies that place the mammillary layer at the intersection of crystal nucleation and matrix control. The present comparison goes further by linking that layer to a specific cross-species glycan-state readout rather than to shell-quality descriptors alone.", [1, 28, 31, 32]),
...
p_disc_mam2 = spara([
    ...
    (" Earlier work had already kept OVAL in view as an abundant eggshell glycoprotein and mineralization candidate, and prior glycoproteomic studies showed that eggshell-matrix proteins can occupy different N-glycosylation states. The advance here is not simply that additional glycopeptides were detected. Rather, the ortholog-resolved cross-species comparison identified which glycan states aligned most clearly with phenotype and carried those assignments into structural and mechanical interpretation.", [4, 18]),
    (" Earlier chicken studies established a glycosite foundation for OVAL and identified glycosylated Asn in OC116. The present dataset resolved dominant glycan classes on the corresponding OVAL ortholog sequons carried into structural modeling (G. gallus N293; A. platyrhynchos and C. livia N97). Relative to earlier site-detection studies, the present comparison extends breadth across species and into glycan-class interpretation rather than merely lengthening a within-species site inventory. OVAL is useful not because it is unique, but because it remains comparable across species while retaining interpretable chemical divergence at the level of specific glycan classes.", [8, 18, 21]),
])
...
p_disc_function = para(
    "Taken together, the comparison converged on a chicken-like eggshell state. Chicken combined the densest mammillary field, the least shielded Ca²⁺-relevant OVAL surface, and the highest local hatching resistance under inside-out loading. This pattern supports the broader inference that chemically specific states on reused matrix proteins can organize mineralized phenotypes more clearly than proteome turnover alone."
)
cite(p_disc_function, [67, 73, 74])

p_disc_selection = para(
    "Duck and pigeon remain essential because they define the bounds of that chicken-like state across both shell structure and ecological-developmental position. Duck combined greater shell thickness with intermediate OVAL accessibility and low τ_max, showing that thickness alone did not recreate the chicken state. Pigeon converged with duck at low τ_max, but did so from a thinner shell and a different mammillary background. Together, these contrasts make chicken the clearest reference state for linking glycan-dependent matrix behavior to eggshell performance across the sampled design space. OVAL glycan state is the most explicit molecular layer through which that state becomes mechanically interpretable."
)

p_disc_biomineral = para(
    "The same analytical sequence may extend beyond avian eggshells. Many biomineralization systems depend on organic matrices that regulate ion access, surface exposure, and mineral nucleation through chemically specific interfacial states rather than bulk composition alone. In that broader frame, the present workflow offers a way to move from glycoproteomic state to surface presentation and then to mesoscale function. Similar scale-bridging problems arise in other mineralized tissues and biomimetic materials. The same logic may also inform regenerative settings in which eggshell-derived materials or membrane proteins are being explored for tissue engineering and bone repair. The study therefore provides a template for testing how chemically specific matrix states organize biomineral behavior across biological and biomedical contexts."
)
...
p_disc_close = para(
    "In summary, this study connected mammillary organization, glycoprotein state, surface accessibility, and local hatching mechanics across three avian eggshells. Chicken defined the high-resistance end of that axis, combining dense mammillary organization, compact OVAL glycans, greater Ca²⁺-relevant surface exposure, and the highest local resistance at the mammillary interface. As comparable glycoform assignments become available, the same strategy can be extended to other abundant eggshell matrix proteins. Together, the morphometric, glycoproteomic, structural, and mechanical evidence identifies OVAL glycan state as the most informative molecular layer linking the chicken-like eggshell state to mammillary organization and local hatching mechanics."
)
