...
p_intro_sig = spara([
    ...
    (" If that structural difference is biologically relevant, it should remain detectable at the hatching-relevant mechanical end point as local resistance at the mammillary interface under egg-tooth-like loading.", [16, 37, 69]),
])

p_intro_gap = spara([
    (" We therefore anchored the comparison to the conserved egg-tooth interface and asked whether glycan-state differences on shared matrix proteins could explain why a common shell-building toolkit yields different Ca²⁺-accessible states at mineralization onset. In that framing, the missing step is a bridge from glycan class to surface presentation on a shared matrix background.", []),
    (" OVAL provided a tractable test case. Its Ca²⁺-responsive surface behavior was already biologically relevant in earlier mineralization work, it remained abundant across species, and its dominant glycan classes could be followed from glycoproteomics into structural modeling.", [4, 18, 29, 42]),
])
...
p_ss1 = smixed([
    ...
    ([(' This broader mapping prioritized the two egg-relevant axes least likely to be secondary: nesting environment, which covaries strongly with terrestrial-to-aquatic habitat use, and offspring developmental state, which spans a continuum from more precocial to more altricial young.', False, False)], [15, 22, 23]),
        ([(' Within this comparison space, ', False, False),
            ...
            (', and ', False, False),
            ('Columba livia', False, True),
                (' were therefore chosen near contrasting regions of those ecological-developmental gradients, so that downstream comparison would be less blurred by intermediate combinations.', False, False)], [3, 22, 23, 41]),
