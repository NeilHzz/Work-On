...
def find_glycoprotein_orthogroups() -> Path:
    scored = []
    for path in ROOT.rglob("Orthogroups.txt.gz.txt"):
        text = path.read_text(encoding="utf-8", errors="ignore")[:12000]
        score = sum(text.count(prefix) for prefix in SPECIES_ORDER)
        if score:
            scored.append((score, path))
    if not scored:
        raise FileNotFoundError("Gly-prefixed Orthogroups.txt.gz.txt not found")
    return sorted(scored, reverse=True)[0][1]


def find_icon_dir() -> Path:
    candidates = [path for path in ROOT.rglob("eggtooth") if (path / "icon_Gallus.jpg").exists()]
    if not candidates:
        raise FileNotFoundError("eggtooth icon directory not found")
    return sorted(candidates, key=lambda path: len(str(path)))[0]


DATA = find_glycoprotein_orthogroups()
