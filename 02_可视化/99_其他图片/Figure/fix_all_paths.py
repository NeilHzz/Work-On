...
PNG  = r"D:\system_folder\Desktop\Work On\02_可视化\Figure\png"

# ── Fig1D ──
with open("Fig1D.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(r"D:\system_folder\Desktop\Work On\01_数据与计算\乳突层形态结构\specie.xlsx",
              ROOT + r"\乳突层形态结构\specie.xlsx")
t = t.replace(r"D:\system_folder\Desktop\Work On\01_数据与计算\乳突层形态结构",
              PNG)
...
# ── Fig2H ── D:/system_folder/Desktop/Work On/01_数据与计算/Ortho/Expansions Contractions Results
with open("Fig2H.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(
    r"D:/system_folder/Desktop/Work On/01_数据与计算/Ortho/Expansions Contractions Results",
...
# ── Fig3A ── BASE = D:\system_folder\Desktop\Work On   and out_path
with open("Fig3A.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(
    r'BASE = r"D:\system_folder\Desktop\Work On"',
    f'BASE = r"{ROOT}"'
)
# Also fix the Orthogroups path – script uses os.path.join(BASE, "Ortho", ...)
# Fix output path
t = t.replace(
    r'out_path = r"D:\system_folder\Desktop\Work On\02_可视化\Sci_Adv_Figure\Fig_glycan_network.png"',
...
t = t.replace(
    r'_SA_FIGURE = r"D:\system_folder\Desktop\Work On\02_可视化\Sci_Adv_Figure"',
    f'_SA_FIGURE = r"{PNG}"'
)
...
with open("Fig4A_C.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(
    r'data_dir = r"D:\system_folder\Desktop\Work On\01_数据与计算\Raw_Data\MS_DATA"',
    f'data_dir = r"{ROOT}\\Raw_Data\\MS_DATA"'
)
t = t.replace(
    'out_dir = r"D:\\system_folder\\Desktop\\Work On\\\u7cd6\u86cb\u767d\u548c\u86cb\u767d\u8054\u5408\u5206\u6790\\Figure"',
    f'out_dir = r"{PNG}"'
)
...
with open("Fig5A_D.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(
    r'FOLDER  = r"D:\system_folder\Desktop\Work On\01_数据与计算\ReGlyco_Ensemble"',
...
with open("Fig5E_H.py", "r", encoding="utf-8") as f:
    t = f.read()
t = t.replace(
    r"CSV_DIR = Path(r'D:\system_folder\Desktop\Work On\01_数据与计算\ReGlyco_Ensemble\csv')",
    f"CSV_DIR = Path(r'{ROOT}\\ReGlyco_Ensemble\\csv')"
)
t = t.replace(
    r"OUT_DIR = Path(r'D:\system_folder\Desktop\Work On\01_数据与计算\ReGlyco_Ensemble')",
    f"OUT_DIR = Path(r'{PNG}')"
)
...
t = t.replace(
    r'OUT_DIR    = r"D:\system_folder\Desktop\Work On\02_可视化\Sci_Adv_Figure"',
    f'OUT_DIR    = r"{PNG}"'
)
