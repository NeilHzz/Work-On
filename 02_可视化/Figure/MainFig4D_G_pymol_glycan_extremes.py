...
def write_pymol_script(jobs: list[dict]) -> Path:
    ...
    script_path.write_text(textwrap.dedent(r'''
        ...
        def scene_overview(job):
            ...
            cmd.set('transparency', 0.18, 'oval and chain A')
            cmd.select('protein_near_glycan', 'byres (oval and chain A within 14 of (oval and chain B))')
            cmd.show('lines', 'protein_near_glycan')
            cmd.color('gray65', 'oval and chain A')
            cmd.set('line_width', 0.75, 'protein_near_glycan')
            color_full_glycan()
            cmd.orient('oval and chain A')
            cmd.turn('x', -8)
            cmd.turn('y', 22)
            cmd.turn('z', -5)
            cmd.zoom('oval and chain A', buffer=58, complete=1)
            cmd.clip('slab', 360)
            add_camera_circle('glycan_locator', job['glycan_center'], max(9.0, job['glycan_radius'] * 1.8))
