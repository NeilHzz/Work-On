...
avonet = pd.read_excel(
    r'D:\system_folder\Desktop\Work On\Work on Desk\Intro\AVONET_Supp1.xlsx',
...
for (_order, _tx, _ty) in _tl:
    if _order not in _order_means.index:
        ax2.text(_BS+3*(_BW+_BG)+0.25, _ty, _order, va='bottom', ha='left',
    ...
    ax2.text(_BS+3*(_BW+_BG)+0.25, _ty,
             '%s  n=%d' % (_order, _n_o),
             va='bottom', ha='left', fontsize=8.5, color=_oc, fontweight=_wt,
             rotation=45, rotation_mode='anchor')
...
for _i, (_cm, _col) in enumerate([(_cm_aq, '#1A4F9A'), (_cm_dv, '#B03010'), (_cm_ds, '#146B50')]):
    ...
    ax2.text(_xb,        -2.35, '0', ha='left',  va='top', fontsize=6.5, color=_col, rotation=90)
    ax2.text(_xb+_BW,    -2.35, '1', ha='right', va='top', fontsize=6.5, color=_col, rotation=90)
...
for _idx, (_ts, _lbl, _col) in enumerate(_CLADES_B):
    ...
    # offset Palaeognathae label further out to avoid overlap with Galloanserae
    _x_off = -1.1 if _lbl == 'Palaeognathae' else -0.45
    if _lbl == 'Palaeognathae':
        ax2.plot([-0.45, -1.08], [(_ylo+_yhi)/2]*2,
                 lw=0.6, color=_col, alpha=0.5, zorder=0)
    ax2.text(_x_off, (_ylo+_yhi)/2, _lbl,
             ha='right', va='center', rotation=90,
             fontsize=10, color=_col, fontweight='bold')

ax2.set_xlim(-2.0, _MD + 18)
...
buf = io.BytesIO()
plt.gcf().savefig(buf, format='png', dpi=160, bbox_inches='tight', facecolor='white')
buf.seek(0)
img = _PILImage.open(buf).rotate(-90, expand=True)

from _save import PNG_DIR, PDF_DIR, SVG_DIR
import base64 as _b64

img.save(os.path.join(PNG_DIR, 'Fig1B.png'), dpi=(160, 160))
img.convert('RGB').save(os.path.join(PDF_DIR, 'Fig1B.pdf'), 'PDF', resolution=160)

with io.BytesIO() as sb:
    img.save(sb, format='PNG')
    sb.seek(0)
    b64 = _b64.b64encode(sb.read()).decode()
w, h = img.size
svg = (f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}">'
       f'<image href="data:image/png;base64,{b64}" width="{w}" height="{h}"/>'
       f'</svg>')
with open(os.path.join(SVG_DIR, 'Fig1B.svg'), 'w') as f:
    f.write(svg)
print('  Saved: Fig1B [PNG/PDF/SVG] (rotated 90° CW)')
plt.close('all')
