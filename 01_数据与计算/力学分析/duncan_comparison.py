...
数据来源：优先读取当前目录的 combined_rcforc_yforce.xlsx（Raw_Data sheet）；
          若 LS-DYNA 结果目录存在则从原始 rcforc 文件读取。
"""
...
EXCEL_SRC   = os.path.join(SCRIPT_DIR, "combined_rcforc_yforce.xlsx")
OUTPUT_PNG  = os.path.join(SCRIPT_DIR, "duncan_comparison.png")
OUTPUT_XLSX = os.path.join(SCRIPT_DIR, "duncan_comparison.xlsx")
...
# ─────────────────────────────────────────
# 数据加载（优先从 Excel，否则读 rcforc）
# ─────────────────────────────────────────
def load_from_excel(path):
    """从 combined_rcforc_yforce.xlsx 的 3Species_Comparison sheet 提取各物种 F_max 和 τ_max"""
    df = pd.read_excel(path, sheet_name="3Species_Comparison")
    # 只取工况行（跳过汇总行 MEAN/STD/t-test）
    cases = df[df["Case"].isin(CASES)].copy()
    F_DATA, TAU_DATA = [], []
    col_map = {
    ...
    for sp in SPECIES:
        fc, tc = col_map[sp]
        F_DATA.append(cases[fc].dropna().values.astype(float))
        TAU_DATA.append(cases[tc].dropna().values.astype(float))
    return F_DATA, TAU_DATA
...
def load_species_rcforc(d, sign, T_mm):
    f_vals, tau_vals = [], []
    for case in CASES:
...
# 判断数据来源
all_dirs_exist = all(os.path.isdir(d) for d in DIRS)
if all_dirs_exist:
    print("[INFO] 从 LS-DYNA 结果目录读取原始数据")
    F_DATA, TAU_DATA = [], []
    for d, s, t in zip(DIRS, SIGNS, T_MMS):
        fv, tv = load_species_rcforc(d, s, t)
        F_DATA.append(fv)
        TAU_DATA.append(tv)
else:
    print(f"[INFO] LS-DYNA 目录不存在，从 {EXCEL_SRC} 读取数据")
    F_DATA, TAU_DATA = load_from_excel(EXCEL_SRC)
