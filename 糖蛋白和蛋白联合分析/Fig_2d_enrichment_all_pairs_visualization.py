...
DATA_DIR       = os.path.join(BASE, "Raw_Data", "MS_DATA")
AVG_FILE = os.path.join(BASE, "Raw_Data", "原始fasta", "Result_AvsG")  # Anas→Gallus outfmt6
CVG_FILE = os.path.join(BASE, "Raw_Data", "原始fasta", "Result_CvsG")  # Columba→Gallus outfmt6
OUT_DIR  = os.path.join(BASE, "糖蛋白和蛋白联合分析", "Figure")
...
def plot_2d_enrichment(sp_ref, sp_comp):
    ...
    ax  = fig.add_axes([0.10, 0.11, 0.60, 0.80])
    ...
    # 目标蛋白
    TARGET_ANNOT = {
        'OVAL':  {'dx': -55, 'dy': -15, 'boxcolor': '#FDDEDE'},
        'OC116': {'dx': -90, 'dy': -18, 'boxcolor': '#D8F0F7'},
        'TRFE':  {'dx': -90, 'dy': -46, 'boxcolor': '#D3F0EB'},
    }
    # Pair-specific label position overrides
    if (sp_ref, sp_comp) == ('Gallus', 'Columba'):
        TARGET_ANNOT['OVAL'] = {'dx': -55, 'dy': 25, 'boxcolor': '#FDDEDE'}
    ...
    if not NOLEG:
        ...
        legend_items = [('circle', '#888888', 7, 'Background')]
        for pname, color in TARGET_COLORS.items():
            if any(df['target'] == pname):
        ...
        y_step, y_pos = 0.075, 0.94
        for mtype, color, sz, lbl in legend_items:
            y_pos -= y_step
            if mtype == 'circle':
                ax_leg.scatter([0.13], [y_pos], c=color, s=sz**1.8, zorder=5,
                               edgecolors='white', linewidths=0.5)
            elif mtype == 'dash':
                ax_leg.plot([0.04, 0.22], [y_pos, y_pos], '--', color=color, lw=1.4)
            elif mtype == 'shade':
                rect = Rectangle((0.04, y_pos - 0.018), 0.18, 0.036,
